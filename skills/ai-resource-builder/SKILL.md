...
1. **Grep the invariant stem, never the templated form.** Search the bare filename stem (e.g. `session-plan`), not a placeholder spelling (`session-plan-${MARKER}`, `{MARKER}`, `$MARKER`) — placeholder variance hides consumers. Run `grep -rn '<stem>' .claude/ docs/ skills/ workflows/ templates/ CLAUDE.md`.
   ...
   **State your instrument — this gate's output is an absence-claim, and a blind scan here ships a broken rename.** The command above names subdirectories and is therefore immune to the shadowed ambient `grep`. **Do not "simplify" it to a dot-rooted walk** (`grep -rn '<stem>' .`): that form silently skips every gitignored path, so a consumer becomes invisible and the inventory reads clean. Full properties and the rule: `docs/audit-discipline.md` § Absence-claims. If a zero-or-near-zero result is about to authorise a removal, prove the instrument first with `. logs/scripts/search-canary.sh`.
