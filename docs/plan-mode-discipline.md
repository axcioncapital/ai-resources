...
**Plan-mode approach selection:** When exploring implementation approaches in plan mode, pick the recommended approach and write the plan around it. Use `AskUserQuestion` only for genuinely ambiguous *requirements* (missing output definition, contradictory constraints) — not to ask the operator to choose between options where Claude already has a clear preference. Present the chosen approach; the operator can redirect via ExitPlanMode approval or a follow-up message.
