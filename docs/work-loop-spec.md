...
- **A specific observed repository fault** — diagnosis and bounded repair belong to Codex `$diagnose-and-fix`. Open-ended "what is wrong here" work belongs to `/audit-repo`, `/repo-dd` or `/consult`. `/work-loop` runs a need someone has already framed; it is not a discovery instrument.
...
- Specific fault resolution (Codex `$diagnose-and-fix`) and open-ended diagnosis (the audit family).
