...
4. **Qualify new resources through `/develop-ai-resource`; build with the canonical pipelines.** `/develop-ai-resource` is the standard qualification path for **creating** a new durable resource — it answers *should this exist, and in what form*, and can end in no build, reuse, an operating change or a deferral. `/create-skill` bounces an unqualified brief back to it once, so new-skill work reaches the build engine through it.
   ...
   **Its operating-outcome sibling is `/work-loop`,** which develops operating capabilities inside projects that already exist (method: `skills/capability-development/SKILL.md`). `/work-loop` owns the operating outcome; `/develop-ai-resource` owns the artifact — the skill is not the capability, it is one implementation component. A brief arriving from `/work-loop` carries `**Capability:**` and `**Settled upstream:**`. Those labels are a **claim** of upstream qualification, not proof of one: Step 1.0 verifies the claim against the named capability record — it must resolve to exactly one file, sit at `projects/{p}/development/{slug}.md` with capability-record frontmatter, agree with the identity the brief supplies, and carry an ACTIVE status — and only then accepts that qualification rather than repeating it. A brief that fails any check is reported as a **malformed upstream handoff** and qualified in full.
   ...
   The build engines are `/create-skill` for new skills, `/improve-skill` for modifications, `/migrate-skill` for converting existing prompts. These pipelines include QC gates — skipping them means skipping quality assurance.
