...
3. **Capture the need, then route to the pipeline.** When working in a project and a skill gap surfaces, use `/request-skill` to write a structured brief to `ai-resources/inbox/`, then hand off to `/develop-ai-resource` in the same session — it establishes whether a durable resource is warranted and which mechanism fits, then hands a qualified brief to the build pipeline. The `ai-resources/` directory is connected via `--add-dir` — no session switch needed.
...
4. **Qualify new resources through `/develop-ai-resource`; build with the canonical pipelines.** `/develop-ai-resource` is the standard qualification path for **creating** a new durable resource — it answers *should this exist, and in what form*, and can end in no build, reuse, an operating change or a deferral. `/create-skill` bounces an unqualified brief back to it once, so new-skill work reaches the build engine through it.
   ...
   **Improvements are not routed through `/develop-ai-resource`.** `/improve-skill` remains directly available and is the ordinary path for an already-identified skill: its mechanism question is settled, so calling it directly is correct, not a bypass. Take an improvement through `/develop-ai-resource` first when it is large, contested, or might really be a new resource.
