...
3. **Capture the need, then route to the pipeline.** When working in a project and a skill gap surfaces, use `/request-skill` to write a structured brief to `ai-resources/inbox/`, then hand off to `/develop-ai-resource` in the same session — it establishes whether a durable resource is warranted and which mechanism fits, then hands a qualified brief to the build pipeline.

   **Same-session handoff depends on the sync, not on `--add-dir`.** Connecting `ai-resources/` via `--add-dir` grants file access and loads its `.claude/skills/`; it does **not** load `.claude/commands/`. The pipeline commands run in a project session only when they are present in that project's own `.claude/commands/` — the shared-resource sync places them there as symlinks to canonical. Where they are absent, `/request-skill` still writes the brief to `inbox/`, and it is fulfilled from a session that has the commands.

4. **Qualify new resources through `/develop-ai-resource`; build with the canonical pipelines.** `/develop-ai-resource` is the standard qualification path for **creating** a new durable resource — it answers *should this exist, and in what form*, and can end in no build, reuse, an operating change or a deferral. `/create-skill` bounces an unqualified brief back to it once, so new-skill work reaches the build engine through it.

   **Its operating-outcome sibling is `/work-loop-v2`,** which carries bounded repository and project work that no specialist owner covers. `/work-loop-v2` owns the operating outcome; `/develop-ai-resource` owns the artifact — the skill is not the capability, it is one implementation component. The boundary is sequential, never simultaneous: while the operating outcome is unresolved the Work Loop owns it, and once the open question is whether a *durable AI artifact* should exist, the Work Loop routes that question here.

   **No component emits an upstream-qualified brief today.** `/develop-ai-resource` Step 1.0 accepts a brief carrying `**Capability:**` and `**Settled upstream:**` as pre-qualified, but those labels are a **claim** of upstream qualification and never proof of one. The producer was to ship with the Work Loop v1 capability route; that route was retired (v1 command deleted 2026-08-06, v1 Codex controller skill retired 2026-08-11), and Work Loop v2 does not emit the fields either. So the clause is dormant producer-side and live consumer-side: Step 1.0 verifies the claim against the named capability record — it must resolve to exactly one file, sit at `projects/{p}/development/{slug}.md` with capability-record frontmatter, agree with the identity the brief supplies, and carry an ACTIVE status — and a brief that fails any check is reported as a **malformed upstream handoff** and qualified in full. Since no proven producer exists, that is the expected path for any brief that arrives carrying the labels.

   The build engines are `/create-skill` for new skills, `/improve-skill` for modifications, `/migrate-skill` for converting existing prompts. Each runs the **skill-evaluation pass** (`skills/ai-resource-builder/references/evaluation-framework.md` — behavioral analysis and convention gate against the drafted skill). That pass is the pipeline's own engine, not a general review stacked on top of one: it produces the evaluation verdict the pipeline exists to emit, and skipping it means skipping the pipeline's output. The change itself is reviewed once, proportionally, per `qc-independence.md` § The rule.

   **Improvements are routed by whether the mechanism is settled.** Go straight to `/improve-skill` when the skill, the requested improvement and the mechanism are all already identified and settled — that is the ordinary path, not a bypass. Qualify through `/develop-ai-resource` first when the underlying need, scope, mechanism or system fit is materially uncertain or contested, or when the improvement may justify a different or materially expanded resource. `/develop-ai-resource` is not mandatory or standard for every material improvement.

5. **Graduation requires confirmation from every existing canonical consumer.** Before graduating a project fork to canonical (i.e., extending the canonical contract with content that originated in one project), enumerate every existing canonical consumer (other projects that consume the resource via symlink or import) and confirm with the operator that each consumer is OK absorbing the change. Record the confirmation in the commit message. **Reason:** `projects/repo-documentation/vault/principles/principles.md § DR-7 — Generalize only when a second confirmed consumer exists` (workspace-relative path; the principles vault is not inside this repo). The "one confirmed consumer + one non-consenting consumer about to absorb the result" pattern is the failure mode this rule prevents — project IP silently spreading into canonical and propagating to other projects without their review. Parameterizing the project-specific value through a project-config knob (so each consumer chooses its own value) is structurally cleaner than graduating the value itself; prefer that path when the value is the project-specific part.

6. **Generalization-residue verification is mandatory at Step 5.5** (added 2026-05-29; Friday-checkup session-qc-pipeline #4). After `/graduate-resource` Step 5's main-agent self-check, an independent fresh-context subagent grep-scans the generalized file for residue from the source project's `CLAUDE.md` (project names, hardcoded paths, domain terminology). On residue found, a fail-and-revise loop (capped at 2 passes) re-runs Step 4 with the residues as explicit revision targets. The cap is this check's own: residue detection is a mechanical grep with a converging target, so a third pass against a structural mismatch is waste. It is not a general review round and does not interact with `qc-independence.md` § The rule. **Reason:** main-agent self-check on its own output systematically under-detects what reads as generic to the author but as project-specific to a fresh reader. The subagent provides the independent perspective; the loop cap prevents infinite revision against structural mismatches. Boundary: this check verifies *generalization*; *placement* remains owned by `Step 3a` (plan-time) and `Step 5a` (end-time) via `docs/placement-verifier.md`. If both fire, placement-verifier wins on placement-related findings; the residue check defers.
...
7. **Complexity budget — no new component that doesn't earn its keep.** No new command, agent, mandatory stage/gate, or permanent always-loaded document may be introduced unless it clears **at least one** prong:
   ...
   **Inflow rule — replacement + invocation path (adopted 2026-07-13, RR-05).** A sharpening of question 5, not a new rule:
   ...
   A **written design principle, not an automated gate — build no checker for it** (RR-05 is explicit). Evidence it is needed: `audits/friday-checkup-2026-07-03.md:152` (*"new commands ship faster than they are wired into pipelines or retired"*, +12 since 2026-06-05); `logs/coaching-log.md:197` (*"if the only way it runs is you remembering it, wire it or defer it; don't ship a memory-dependent orphan"*); and `audits/lean-repo-2026-07-13.md`, which found **89 commands, +5 in 10 days, 0 ever removed**, six commands with zero references and zero invocations, and **two repo-design diagnostics (`/lean-repo`, `/architecture-review`) that had never run because neither had an invocation path.** `/lean-repo` was itself the exemplar: it had to be fired by a line item in a redesign programme to run even once.
   ...
   **Distinct from `docs/materiality-bar.md`:** that bar governs which *review findings* become Findings; this gate governs whether a new *resource* gets created — a different lifecycle stage. **Operationalizes at creation time** the AP-7/DR-7 speculative-abstraction ban (generalize only on a second confirmed consumer) and **OP-12** (closure before detection): a new *detection* component — an audit, scan, flag, or finding-generator — must additionally ship its **closure channel** (the thing that acts on what it finds) in the same change to clear the gate. A new component that fails prong (a), leans only on prong (b), and is a detector still needs its closer; shipping the detector alone is an OP-12 violation.
   ...
   If a component is introduced despite failing the gate, that is a **loud, recorded principle exception (OP-11)** — a deliberate decision logged in `logs/decisions.md` with its rationale, never an in-line "it's fine actually" assertion. The risk-aware review carries principle alignment as one of its seven dimensions, so this gate is applied at land-time by that review (`qc-independence.md` § Risk-aware review) — a new command or skill is in a structural class, so it always gets one. `/leverage-idea` applies it at proposal-time.
