...
4. **Route new resource decisions through `/develop-ai-resource`; build with the canonical pipelines.** `/develop-ai-resource` is the entry point when the question is *should this exist, and in what form* — it can end in no build, reuse, an operating change or a deferral. The build engines are `/create-skill` for new skills, `/improve-skill` for modifications, `/migrate-skill` for converting existing prompts. These pipelines include QC gates — skipping them means skipping quality assurance. An improvement to an already-identified skill may go straight to `/improve-skill`: the mechanism question is settled there, so only the evidence check applies.
