...
### Step 3 — RETIRED 2026-07-30; the reasoning moved with the scan, and is kept because the traps did not expire

> **The step this section documents no longer exists.** `/prime` Step 3's bounded urgent-log scan was
> replaced on 2026-07-30 by `logs/scripts/promote-findings.sh`, run once at wrap instead of at every
> orientation (stream `2026-07-30-prime-session-entry-ownership`, S3). **Read this section as the
> promotion sweep's rationale**, not as `/prime`'s: every trap below — the anchor widenings, the
> declaration-line exclusion, the unclassified-tail argument, the `medium-high` policy — transferred to
> that script intact, because they are properties of the *log format*, not of where the scan ran. The
> one paragraph that did not transfer is the `friction-log.md` one; see its own note below.
>
> The cost history is retained deliberately. It is the reason promotion is a wrap-time write rather
> than an orientation-time read, and a future session proposing to "just grep the backlog at `/prime`"
> needs to meet it.
...
**Why `friction-log.md` was scanned differently — and why that half did NOT transfer.** It has no severity field: its severity words are free text inside prose bullets, and its resolution stamps (`— **Resolved:**`, `[FADING-GATE] verified`) sit on the *same* line as the finding, which is why a same-line `grep -v` worked there and would not work on the schema'd log. Its hits were **candidates to judge, not findings** — incidental matches were expected (a shell variable named `HIGH`, a quoted phrase) and were cheap for a session to discard in context, because only the matching lines came back.

**A script cannot judge, so the promotion sweep does not inherit that keyword fallback.** Measured 2026-07-30 against this repo, the old grep returned 3 hits and all 3 were prose: the word `HIGH` inside an allocator comment, "the open HIGH item of 2026-07-14" inside a narrative, and a "4 HIGH research-workflow relays" aside. Promoting them would have put three non-findings on the task menu with nothing to discard them. `promote-findings.sh` therefore reads `friction-log.md` through the **same severity-field test** as the schema'd log — live code, currently matching zero entries — and the day friction entries start carrying `**Severity:**` they promote with no change. The consequence, stated plainly: **an un-tagged friction entry reaches no queue at all.** Before, a session's judgement could rescue it from the grep output. Closing that gap needs a severity field in the friction-log schema, not a looser regex.

**The `medium-high` near-miss (2026-07-24) — why the tier's menu-reach is a policy question.** Step 3's prose once read *"include only if it carries a HIGH-severity marker"* and *"exclude anything marked `LOW` or `MED`"*, naming `medium-high` in **neither** clause while the anchor matched it. That silence read as an anchor-vs-filter contradiction, and a change was proposed deleting `medium-high` from the anchor — a ~70% emit reduction — on the premise that the filter already discarded it. **It does not, and three other files say so explicitly:** `wrap-session.md` Step 12e (*"Only `high` and `medium-high` reach the `/prime` task menu"*), `.claude/agents/session-feedback-collector.md:138` (*"`high` / `medium-high` → reaches the `/prime` task menu"*), and `logs/improvement-log.md:13`, the log's own schema. The anchor was right; the prose was wrong, and was corrected rather than the anchor. the risk review returned RECONSIDER on the deletion — report at `audits/risk-checks/2026-07-24-narrow-prime-step3-severity-anchor-medium-high.md`. A large emit is a signal that too many `medium-high` items are genuinely open; the remedy is backlog triage, not a quieter scan. **This policy transferred to `promote-findings.sh` unchanged, and so did its lockstep requirement** — narrowing the tier still means editing the two writer-side files and the log's own schema in the same commit, plus a `logs/decisions.md` record.
