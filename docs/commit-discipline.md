...
> **Rationale home only.** The **rules** — the behind-check, the pull command, the mid-rebase abort, the autostash-pop detection, the four exit-code cases, the unpushed clause — live in `.claude/commands/prime.md` Step 0, which is what executes them. This section holds only *why they are shaped that way*, so the executable step stays short. Relocated from `prime.md` on 2026-07-29 (stream `2026-07-29-prime-minimum-responsibility`, Slice 4). **A rule must never be moved here** — a rule read from a doc either costs the same tokens it was meant to save, or silently stops firing.
