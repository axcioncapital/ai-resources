...
## Orientation pull (`/prime` Step 0)

> **Rationale home only.** The **rules** — the behind-check, the pull command, the mid-rebase abort, the autostash-pop detection, the four exit-code cases, the unpushed clause — live in **`logs/scripts/prime-sync.sh`**, which is what executes them; `/prime` Step 0 is now a call site. This section holds only *why they are shaped that way*. Relocated from `prime.md` on 2026-07-29 (stream `2026-07-29-prime-minimum-responsibility`, Slice 4); the rules themselves moved from the prompt into the script on 2026-07-30 (stream `2026-07-30-prime-session-entry-ownership`, S4), so they are now validated by **running** rather than by reading — tripwire `logs/scripts/prime-sync.test.sh`. **A rule must never be moved here** — a rule read from a doc either costs the same tokens it was meant to save, or silently stops firing.

**Why the behind-check runs first, and why it is not merely an optimisation.** `/prime` fetches and asks `git rev-list --count HEAD..@{u}` *before* running any rebase, and skips the pull entirely at `BEHIND=0`. The incident (2026-07-14, S5 → fixed S8): `/prime` ran `pull --rebase --autostash` unconditionally, hit a content conflict in `logs/session-notes.md`, and **halted orientation mid-rebase** — leaving the repo in a `rebase-in-progress` state at the very start of a session. **The conflict was entirely spurious: `origin/main` had not moved at all** (`[ahead 5]`, zero behind), so there was nothing to pull. `--rebase` nonetheless replayed local commits — including a local **merge** commit — flattening it and re-applying conflicts that the merge had already resolved. **This repo creates local merge commits by design** (`/close-worktree-session` Step 4 makes them), so rebasing local history re-litigates settled conflicts, and it will recur on any merge-bearing history. The behind-check removes the whole class: with nothing to pull, there is nothing to rebase.

**Why `--rebase --autostash` is explicit rather than left to config.** Both flags are stated in the command, not delegated to per-machine `pull.rebase` / `rebase.autoStash` settings, so the behaviour cannot vary by machine. `--autostash` means a dirty working tree from a prior same-day session is stashed, rebased over, and popped back in one command — the rebase no longer refuses to start, which removes a failure-and-recovery round-trip at every session open.

**Why a half-rebased repo at session start is an incident, not a no-op.** When the pull conflicts mid-flight, the sync owner aborts the rebase and keeps orienting on the pre-pull state rather than stopping to resolve it. Orientation is not the place to resolve a merge: the operator came to start work, and a repo left mid-rebase at that moment converts a no-op into an incident they must clear before they can do anything at all. The pre-pull state is a perfectly good state to work from; the ⚠ line in the Step 6 brief is what tells them it happened.

**Why the autostash-pop conflict needs detection of its own.** With `--autostash`, the history rebase can succeed (**exit 0**) while the *pop* of the stashed dirty tree conflicts. Git prints `Applying autostash resulted in conflicts. Your changes are safe in the stash.` and still returns 0 — so a classifier keyed on the exit code alone reports `updated` while the working tree silently holds conflict markers. That is why the sync owner detects this case on three OR-ed signals (pull output text, a residual `autostash` stash entry, a `UU` path in `git status --short`) and classifies it *before* the two exit-0 cases. Three signals rather than one because git's wording is not a stable interface.
...
**Footprint source and fail-open.** The hook resolves this session's marker (per-id oracle `logs/.session-marker-${CLAUDE_CODE_SESSION_ID}` → shared `logs/.session-marker` fallback) and reads **two** bullets under this session's `## DATE — Session S{N}` header in `logs/session-notes.md` — `- Files in scope:` (the same bullet `/session-start` writes and `concurrent-session-check.md` reads) and, since 2026-07-19, `- Required outputs:`. If that footprint is absent, or reads `(inferred)` / `(none stated)` / has no concrete path, the hook **fails open** — it emits a soft non-blocking warn and allows the commit (a guard that blocked on its own parse failure would be worse). **Known blind spot:** a primed-but-not-planned session, or one with an `(inferred)` footprint, gets *no* protection from this tripwire — the same gap `concurrent-session-check.md` documents as its #1 failure. Declare a concrete `Files in scope` (run `/session-start`/`/session-plan`) to arm the guard.

**Why both bullets, and why asymmetrically (2026-07-19).** `/session-start` Step 2.5(b) hard-rejects a not-yet-existing path from `files_in_scope` and routes it to `- Required outputs:`. Reading only the first bullet therefore blocked sessions that followed the schema *exactly*: on 2026-07-19 a session declared its two created files under Required outputs and the guard reported them as foreign, naming the concurrent-session contamination pattern on a solo session. Worse, the block message told the operator their "footprint is too narrow" — pressure to move created files back into `Files in scope`, which Step 2.5(b) forbids. Two rules in the same harness contradicted each other, and the guard trained operators to violate the adjacent one.

The two bullets are **not** read symmetrically, and the asymmetry is load-bearing:

- `files_in_scope` is hard-validated (shape + existence, both HARD REJECT) and alone decides **whether the guard is armed at all**.
- `required_outputs` has **no validation of any kind** — Step 2.5 validates `work_scope`, `exit_condition`, `files_in_scope` and `stop_if`, and this is not one of them. Real entries on disk are prose. Its tokens are therefore shape-filtered individually, and it may only **widen** an already-armed guard, never arm a disarmed one.

That keeps the union **monotonic**: it can turn a false block into a pass, never a pass into a block. Letting `required_outputs` arm the guard would hand a near-empty footprint to an armed tripwire and hard-block solo sessions. Both halves — the shape filter and the do-not-union note at the `no_concrete_footprint` gate — are required to hold the property. Verified end-to-end against a 10-case fixture plus the three real on-disk field shapes.
...
**Narrow exception (2026-07-03, incident `9660bf2`).** "Benign" above assumes no live foreign session — with a concurrent session actually active, an exempt file can still be that session's in-flight work, and a bare `git commit` folding it in silently misattributes it. The hook now warns (never blocks) in exactly that case: a `git commit` would include exempt-but-outside-footprint files AND a foreign session is live in this checkout (a foreign per-id marker is present AND — process-grounded 2026-07-18 — a foreign Claude CLI process still has its cwd inside the checkout; see `docs/session-marker.md` § Concurrent-session detection). The exempt-list's benign-by-default classification is unchanged; this only adds a signal for the one case where "benign" isn't guaranteed.
...
A prose warning saying exactly this — *"Never remove a worktree a live session still occupies"* — **already existed** in `new-worktree-session.md` and `close-worktree-session.md` before that incident, **and it did not fire**, because the destructive command was assembled directly in a session plan and never touched either file. Adding a fourth prose location would have changed nothing about that causal path. **A rule you must remember to read is not a control; it is a wish.** The 2026-07-14 plan-time risk review said so in as many words, and the doc-only design was replaced with the hook.

**A pre-implementation review is also the wrong home, and this is not a matter of taste.** It runs *before* the work and reads the repo *at rest*. A session can go live between the gate and the act — which is not hypothetical: on 2026-07-14 (S4) the target worktree was verified clean and idle at 10:50, and was **occupied by a new live session by 11:10**, twenty-five minutes later, while the fix for this very defect was being written. Putting the probe in a pre-implementation review relocates the bug one layer up. **Liveness must be probed at execution time, by the executor, or it is a reading of a moving system.**

**The generalisable statement.** Every other gate in this repo (the risk-aware review, `/blindspot-scan`, `/lean-repo`) inspects the artifact **at rest**. This hazard is the artifact **in motion**. A file census cannot see a running process. *A clean worktree is not an idle worktree.*
