...
3. Baked-in exclusions are skipped: `EXCLUDE_COMMANDS = "new-project deploy-workflow pipeline-review scope-project lean-repo"`, `EXCLUDE_AGENT_GLOBS = "pipeline-stage-* session-guide-generator pipeline-review-* scope-*"` — these are ai-resources-meta and never belong inside a downstream project.
   - **Workspace-root exception.** The exclusions in rule 3 are *not* applied when the hook syncs the workspace root itself (detected as: `ai-resources/` is a direct child of `$CLAUDE_PROJECT_DIR`). The root is a valid place to run these commands — `/new-project` states this in its own CWD guard (`new-project.md`: running from the workspace root is valid; only running from inside `ai-resources/` is blocked). Treating the root as a downstream project made `/new-project` and its four siblings unreachable there. The excluded **agents** are exempted at the root too, not just the commands: `/new-project`, `/pipeline-review`, and `/scope-project` delegate to `pipeline-stage-*`, `pipeline-review-*`, and `scope-*` agents respectively, so a command-only exemption would leave them visible in the menu but failing mid-pipeline on subagent spawn.
