...
3. **Dedup before append (grep-first, read-narrow).** Grep each candidate's root-cause terms + principle ID against the **active** `improvement-log.md` (and `friction-log.md` for friction candidates), then read only the ~10 lines around any hit to confirm; do not re-log a signal that already has an active entry. Do NOT full-Read the logs and do NOT scan `improvement-log-archive.md` — archived entries are resolved, so a recurrence is a new signal, not a duplicate (and the archive is denied to `Read` by `settings.json` regardless).
