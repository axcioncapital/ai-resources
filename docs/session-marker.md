...
`/prime` Step 1a carries two concurrent-session signals of its own. They are planning-time companions to the hook above, not duplicates of it — the hook says *"a session is live, isolate"*; Step 1a says *"a session is live, so check your next task will not collide before you pick it."* The **rules** live in `prime.md` Step 1a; this subsection is their rationale (relocated 2026-07-29, stream `2026-07-29-prime-minimum-responsibility`, Slice 4).

**Multiple same-day headers are the EXPECTED shape, not a hazard.** Under marker-scoped session writes each session writes its own marker-bearing header `## YYYY-MM-DD — Session ${MARKER}`, so several per day is normal and Step 1a must **not** emit a `⚠` for it (`principles.md § AP-10` — no error handling for impossible-or-normal scenarios). `SIBLING_COUNT` therefore exists only as a *gate*, never as its own display line: it gates the shared-dir advisory, and it informs the `/concurrent-session-check` nudge. Its standalone brief line was removed in the 2026-07 brief-simplification pass.
...
**Why the liveness nudge uses the per-id marker set rather than `SIBLING_COUNT`.** `SIBLING_COUNT` counts same-day *headers*, which cannot distinguish a live session from one that already wrapped — the marker is date-pruned, not liveness-pruned — so it is the wrong gate for a "you have a concurrent session right now" nudge. Step 1a reuses the per-id marker set instead. Since 2026-07-18 that set is pre-cleaned by `detect-concurrent-session.sh` (which runs at every session start, i.e. before any `/prime`, and prunes markers whose session provably has no Claude CLI process left in this checkout), so a surviving foreign marker at Step 1a time is much more likely to be genuinely live. **The hook is the authority on liveness; Step 1a's own read stays today-scoped and heuristic**, and degrades safe to no nudge when `CLAUDE_CODE_SESSION_ID` is unset on an older CLI.
