...
- `ai-resources/.claude/hooks/detect-concurrent-session.sh` (+ byte-identical project copies: `projects/positioning-research/.claude/hooks/`, `projects/research-pe-regime-shift-advisory-gap/.claude/hooks/`) — SessionStart hook; reads the today-dated per-id marker set (`logs/.session-marker-*`, excluding this session's own) as a **liveness signal** to fire the same-checkout sharp nudge only when an un-wrapped foreign session exists here. Depends on the `/wrap-session` Step 13 teardown to keep the signal accurate; falls back to the legacy shared-marker heuristic when the per-id oracle is unpopulated. **All three copies must be updated in lockstep** (auto-sync does NOT cover hooks). A silent non-match degrades to the soft machine-wide warning, never a block (SessionStart cannot block — see the hook header).
...
**Same-checkout liveness discriminator (2026-06-10 — Fix 1).** To decide between the SHARP same-checkout nudge and the SOFT machine-wide warning, the hook reads the today-dated per-id marker set (`logs/.session-marker-*`, excluding this session's own `${CLAUDE_CODE_SESSION_ID}` file). Because `/prime` creates the per-id marker and `/wrap-session` Step 13 removes it, a today-dated foreign per-id marker means an **un-wrapped ≈ live foreign session in THIS checkout** → SHARP. This replaced the original signal (a today-dated *shared* `logs/.session-marker`), which is DATE-pruned not LIVENESS-pruned and therefore stayed set after this operator's OWN earlier session wrapped — false-firing the sharp nudge on every solo same-day re-open. When the per-id oracle is unpopulated the hook falls back to the legacy shared-marker heuristic so the safety net is never lost. Both reads are read-only; the hook still inherits no clobber bug.
