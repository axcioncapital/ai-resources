...
**Canonical writers already comply.** `/prime`'s shared **Step 8k** (referenced by 8a/8b/8c) writes both files together (shared `echo` immediately followed by the `[ -n "${CLAUDE_CODE_SESSION_ID}" ] && echo … > "logs/.session-marker-${CLAUDE_CODE_SESSION_ID}"` line). (Step 8k no longer prunes orphans — see **Lifetime** above; liveness-grounded pruning moved to `detect-concurrent-session.sh`, 2026-07-18.) This invariant exists so that any **deviation** — a hand-rolled or alternate setup that writes one file — is recognised as a contract violation, not treated as a valid degraded state. Do not author a marker-setup path that writes the shared file without the paired per-id write.

---

## Marker allocation (the WRITE path — `/prime` only)

**Do not confuse this with § Marker resolution below.** *Allocation* chooses a **new** `S{N}` at session start and is performed by `/prime` alone (shared **Step 8k**, referenced by 8a/8b/8c). *Resolution* is how every **other** consumer looks up the marker it was already given. Changing one does not change the other; `/prime`'s Step 8k allocation block and this section must stay in lockstep.
