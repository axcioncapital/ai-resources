...
Run `/pipeline-review` once per week to give 1–3 critical pipelines a deep System-Owner-grounded design review (innovations, leanness, brokenness, cross-resource interactions). Memo only — fixes happen in a separate session. Registry: `audits/pipeline-review-registry.md`. Distinct from `/audit-critical-resources` (drift) and `/friday-checkup` (housekeeping).
