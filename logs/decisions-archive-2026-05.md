...
**APPLIED 2026-06-01 (S3) — shipped 11dfd92 (2026-05-29, S6 Wave 6 #16).** The auto-triage capability this item scheduled is live: `friday-act.md` sub-step 13a (auto-triage default HIGH→f, MED→f, LOW→d per Item 10) + revised sub-step 14 (Enter accepts default, paste overrides), inherited by the Step 3.5d/3.5f loops. Inline /risk-check GO, all five dimensions Low. Note on path: implemented via **direct-edit** to `friday-act.md`, NOT via `/improve-skill` (the pipeline targets SKILL.md; command files lack the required `model:`+`effort:` frontmatter) — so the scheduled "dedicated /improve-skill session" did not occur as titled, but the operator directive ("triage the follows AUTOMATICALLY") is satisfied. Disposition flipped from the original SCHEDULE-DEDICATED below (QC surfaced the stale disposition in S2).
...
### 2026-05-29 — /cleanup-worktree S9 — three policy alternatives encoded with reversibility

**Context.** Session S9 ran `/cleanup-worktree` on the ai-resources working tree (6 dirty paths). Independent triage surfaced three first-class operator-policy alternatives that the cleanup-time picks would encode — these were policy choices the operator had previously left as Open Questions (Alt A surfaced in S6 explicitly) or that matched established conventions implicitly (Alt B, Alt C). All three are recorded so future cleanups have an audit trail of the encoded policies and can revisit them without re-deriving the rationale.

**Decision 1 — `output/` policy: gitignore at ai-resources level.** Cleanup commit `f1edccc`. The S6 Open Question read: "Heterogeneous output/ git-tracking across projects — pack persistence varies. … no policy enforced." The cleanup picked gitignore because (a) S7 and S8 mandates already labeled their pack paths "(untracked)" — de facto policy existed at the mandate level; (b) without gitignore every future engine run re-dirties `git status`, creating recurring cleanup work that is itself a friction class; (c) the change is one-line reversible. Alternative considered: leave `output/` as untracked-but-not-ignored and defer to a dedicated policy session. Rejected because the de facto pattern already supplied the policy and the cost of recurring cleanup work was greater than the cost of one reversible line.

**Decision 2 — Inbox briefs: `mv` to `inbox/archive/` without prior commit.** Briefs `inbox/context-engine-brief.md` and `inbox/context-engine-session-pairing.md` moved to gitignored archive directory. The triage observed that the briefs never enter git history under this approach; the fulfillment commits (`7dc5e6e` Phase 1, `e774eb5` Phase 2) document the work but not the originating brief text. The cleanup picked `mv`-only because (a) `.gitignore` lines 36–37 explicitly label the archive pattern "Archived inbox briefs" — the established convention is briefs do NOT enter git history; (b) the substantive brief contracts were absorbed into the produced artifacts (engine command + agent + schema docs), all of which ARE in git; (c) adding a "preserve briefs in git" commit would deviate from the operator's established pattern without solving a real loss scenario. Alternative considered: stage + commit briefs before `mv`. Rejected on convention-deviation grounds.

**Decision 3 — `logs/session-notes.md`: defer-to-wrap rather than commit during cleanup.** The S9 `/prime` header was the file's only modification at cleanup time. Cleanup picked defer because (a) every S3–S8 wrap commit in `git log` covers both header and body in a single commit — committing during cleanup would force `/wrap-session` to produce a second wrap commit; (b) the deferred file is the deliberate target of `/wrap-session` at session end. Triage agreed with the plan.

**Alternatives broader than per-decision.** The triage was rubric-bounded; it did not propose a "defer all three to a dedicated policy session." That alternative would have been valid — the cleanup session implicitly turned itself into a policy session for three points — but it was not adopted because the reversibility of each pick capped the downside cost and the cleanup-vs-policy distinction is a stylistic one, not a structural one.

**Trigger to revisit.** Any of the three picks is fair game for revisit at next `/friday-act` or in a future cleanup. The plan file at `/Users/patrik.lindeberg/.claude/plans/quizzical-wiggling-robin.md` Section 8 captures the full rationale and the alternatives-considered list for each pick.
