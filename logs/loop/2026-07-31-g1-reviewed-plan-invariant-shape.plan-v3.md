...
# PLAN v3 — Slice 1: G1 reviewed-plan integrity

Revision: **v3**, immutable. Supersedes plan-v2 for execution; v2 and v1 are retained unedited.
...
| Plan v2 — reviewed | `logs/loop/2026-07-31-g1-reviewed-plan-invariant-shape.plan-v2.md` | `bb476184c57d04ee7b0a96645fa655435652c2a9` | `90f0b931272dead2dedb679c8b5cc834b680a3d7` |
...
| Adjudication | `logs/loop/2026-07-31-g1-reviewed-plan-invariant-shape.evidence.md` | `854d977fbb4133e4a6e4b129ef9e2950bca4016b` | `4d0c6cd0b675486f9c61577882ee0d53ec09110d` |

**This revision is the single bounded material correction pass** permitted by
`docs/work-loop-repair-workflow.md` §10. It is now consumed. One conditional closure `review-2`
remains, and is justified — all three corrections change matters review-1's verdict rested on.

Binding verified before writing: branch `codex/2026-07-31-g1-reviewed-plan-invariant`, HEAD
`854d977fbb4133e4a6e4b129ef9e2950bca4016b`, base `6050a5b` an ancestor, worktree clean. Authority
blobs re-derived at this HEAD and **unchanged** since plan-v2 §13.6, so every line number cited below
still resolves. No object edit occurs in Shape.
...
minor, binding verification PASS. It recorded no overengineering, no scope drift and no
over-governance, and confirmed the atomic-slice structure, the four-file boundary, the plan-identity
binding, the non-material no-mutation rule, the no-`review-3` rule, the Shape-only `hold-reframe`
reservation, rollback and exclusions as sound. All three findings were adjudicated `fixed`.

| Finding | Defect in v2 | Resolution in v3 |
|---|---|---|
| **G1-RV2-01** | The one-shot malformed-header allowance left no committed trace, so each resumed session could grant another "one-time" repair — an unverifiable cap | **§6.8** — durable `HEADER-REPAIR` receipt carrying the first block verbatim; byte-identity rule |
| **G1-RV2-02** | The G1 package was required to display "review identities" that the plan never defined — a regression against plan-v1 §7.4 | **§6.2** — `REVIEW-PATH` / `REVIEW-COMMIT` / `REVIEW-BLOB` defined, computed and verified |
| **G1-RV2-03** | `hold-reframe` had no capability-record close-and-resume transition, so Tier 3 resume would carry a terminally closed stream | **§6.10** — the full transition, plus one narrow exception to the `stream:` carry rule |

**Operator correction of record on G1-RV2-01.** My first proposed receipt persisted only the verdict,
the finding IDs and the counts. Patrik rejected that as too weak: those can be identical while the
reasoning, evidence or required corrections change underneath them, which would let a materially
different review enter through a formatting allowance. **The receipt persists the complete first
malformed `REVIEW` block verbatim**, and the re-emission must be byte-identical except for the three
`PLAN-*` header lines. Recorded in the Shape evidence at blob `4d0c6cd0…` §7.
...
**Section map from v2**, for a reviewer holding the earlier revision: v2 §6.1 → v3 §6.1 · v2 §6.2 →
v3 §6.3 · v2 §6.3 → v3 §6.4 · v2 §6.4 → v3 §6.5 · v2 §6.5 → v3 §6.7 · v2 §6.6 → v3 §6.8 · v2 §6.7 →
v3 §6.9 · v2 §6.8 → v3 §6.11 · v2 §6.9 → v3 §6.12 · v2 §6.10 → v3 §6.13. New: §6.2, §6.6, §6.10.
...
**Template boundary, re-verified at this HEAD.** `:98` is the sole unit-outcome enumeration. Line 7 is
the separate `status:` axis (`adopted \| keep-local \| closed \| retired \| rejected`) and is **not**
touched — outcome and status are different axes (`docs/work-loop.md:200`). §6.10's transition needs no
new frontmatter key and no new section: `status:`, `reopen_trigger:`, `active_unit:` and `stream:`
already exist at `:6-11`, `## Units` at `:92`, `## Current phase and next action` at `:126` and
`## Pointers` at `:138`. Writing *into* an existing section is not a template change.
...
| 4 | Validate the re-emission against the receipt (below). Valid → transcribe and continue. Invalid → **stop before G1**, leave the unit open, return the §6.7 blocker handoff |

**The receipt** persists, in the append-only evidence file:

- the date, and the statement that the single re-emission allowance is now **consumed**;
- the plan identity the received block named, as received;
- **the complete first malformed `REVIEW` block, verbatim.**

**The re-emission test.** The re-emitted block must be **byte-identical** to the persisted block except
for the three header lines `PLAN-PATH`, `PLAN-COMMIT` and `PLAN-BLOB`, which must be present and
well-formed in the re-emission. Every byte outside those three lines must match exactly. This covers a
header line that was malformed, missing or duplicated in the original.

**Any other difference makes it a new review**, which the header-repair allowance does not permit — a
second review is not in the budget. The executor stops before G1 and returns the blocker handoff.
...
permitted**, and the only remaining moves are a valid header or the blocker stop.

*Why verdict, finding IDs and counts are insufficient, and the verbatim block is required:* those
three can be identical while the reasoning, the cited evidence or the required corrections change
underneath them. A materially different review would then enter through a formatting allowance, which
is the unbudgeted second review the cap exists to prevent. Operator decision of record; recorded in
this unit's evidence at blob `4d0c6cd0…` §7.

*Why this does not contradict §6.4:* §6.4 forbids a malformed header in the **immutable** review
artifact `{unit}.review-{n}.md` (`docs/work-loop.md:142`), where no lawful correction path would exist.
The receipt is written to the **evidence** file, which is **append-only, not immutable** (`:141`) and
...
budget, and does **not** trigger `hold-reframe`. It is capped at one and the cap is now provable from
the repository, so no retry loop exists across sessions.
...
**The conflict, verified at this HEAD.** `docs/work-loop.md:109` and `.claude/commands/work-loop.md:175`
both fix the record's `stream:` as allocated once and carried unchanged. Tier 3 resume takes its
stream from that field (`docs/work-loop.md:256`, `.claude/commands/work-loop.md:32`), and `:37` warns
that allocating instead of carrying splits a stream silently. But `…repair-workflow.md` §10 requires
any continuation after `hold-reframe` to start a **new** stream citing the held one. As written,
resume would carry a stream that `hold-reframe` has terminally closed.
...
1. **`docs/work-loop.md`** — plan identity and binding relation (§6.1); **review identity** (§6.2);
   ...
   byte-identity rule and resume rule (§6.8); materiality and non-material-no-mutation (§6.11);
   lifecycle and no-`review-3` (§6.9), correcting `:102` (§6.13); `hold-reframe` as fifth outcome at
   `:196`/`:198` plus its row in § Closing without a change; **the capability close-and-resume
   transition and the narrow stream-allocation exception** (§6.10); `review-{n}` path (§6.12).
...
| **A8** | Header repair writes a committed `HEADER-REPAIR` receipt carrying the **complete first block verbatim**, before any re-emission is requested |
| **A9** | The re-emission must be byte-identical except the three `PLAN-*` header lines; any other difference is a new review and stops |
...
| **M7** | Header invalid once, corrected on the single permitted re-emit, body otherwise byte-identical | **Proceeds, budget unconsumed** |
| **M8** | Re-emission changes a finding's reasoning while keeping its ID, verdict and count | **Stops — a new review, not a header repair** |
...
| **F8** | A substantively changed review enters through the header-repair allowance |
...
Text and construction checks; the behavioural case belongs to Stage 9.
...
| **V-H2** | Contract states the byte-identity rule and names the three exempt lines | ≥1 match on `byte-identical` | `grep -c 'PLAN-BLOB' docs/work-loop.md` → non-zero |
| **V-H3** | Contract states the resume rule keyed on the committed receipt | ≥1 match on `HEADER-REPAIR` | as V-H2 |
| **V-H4** | M8/M9 are expressible: a body-diff and a post-restart offer both reach a stop | trace both paths through the written steps | — |
...
(Use). M8, M9 and M10 in particular are behavioural: a body-differing re-emission, a post-restart
re-offer and a capability reframe cannot be staged before the slice exists. Declared, not hidden.
...
4. **The byte-identity rule assumes the persisted block is itself faithfully captured** (§6.8). It
   binds the re-emission to what was recorded, not to what the reviewer emitted. Same Slice-4 residual.
...
8. **Line numbers** are as of blobs `88f555e630a4ae898d0eb6d1827d908faf1bf81a` (contract),
   ...
   `f0580c9e98f45232d83d1cf6d707b39c9e186acf` (template), re-derived at HEAD `854d977f` and unchanged
   since plan-v2. Build re-derives them if any file moves.
...
- **G1-RV2-01 / -02 / -03** — all three **closed** by §6.8, §6.2 and §6.10 respectively.

**Budget after this revision:**

| Item | Status |
|---|---|
| One initial independent review | **Consumed** — review-1, commit `96b27e53…`, blob `eb827a67…` |
| One bounded material correction pass | **Consumed by this revision** |
| One conditional closure `review-2` | **Available and justified** — all three corrections change matters review-1's verdict rested on |
...
Settled scope decisions were not reopened: the four-file boundary, plan-v1's stopped status and the
exclusions all stand as written.
