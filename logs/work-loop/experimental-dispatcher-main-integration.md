...
- Claim (7) — `logs/friction-log.md` carries a pre-existing unrelated uncommitted modification: HOLDS — `git status --porcelain` at entry reported exactly ` M logs/friction-log.md` plus the untracked state file; entry `sha256 = c056dbb38da193efe507f5b8794f0f9ba9ef4ed5e37c66ae57aadc56e34ca8f7`, a 623-line insertion over HEAD. See the blocker below: this claim holding is what stops the unit.
  - **That fingerprint moved during the hop, and not by this unit.** After the state-file write, the file is at `968f8bf581744c26021acf3cf96fa697bc7e104d9e06d0426c3f66f358692ba7` — a 625-line insertion. The two added lines are `- 23:08 — logs/work-loop/experimental-dispatcher-main-integration.md`, twice: this checkout's unguarded `log-write-activity.sh` hook recording my own writes to the state file. No `Write`, `Edit` or `git add` in this hop targeted `logs/friction-log.md`; `git diff --cached --name-only` lists nothing, and the file is still ` M` and uncommitted. Quoting a settled post-hop fingerprint would be false evidence — any further write to the state file appends again — so the durable fact is the shape, not the hash: the file's only working-tree change is hook-appended `Write Activity` lines over HEAD, none of them staged or committed. This is the same unguarded hook named in the deferral below, now demonstrated rather than inferred.
