...
- Claim (3): HOLDS, with one measured qualification. In a disposable fixture, `git rev-parse --git-path hooks/pre-commit` returned `.git/hooks/pre-commit` at an ordinary repo root, `../../.git/hooks/pre-commit` from a subdirectory (relative to the git invocation's cwd, matching the existing exclude-path handling), and the main checkout's `.git/hooks/pre-commit` from a linked worktree — the shared common surface, as claimed. The qualification: `git config core.hooksPath` overrides that location, and it **is** set locally in this checkout (`.../ai-resources/.git/hooks`, which happens to equal the default). `--git-path` alone would therefore not be the executing surface wherever `core.hooksPath` points elsewhere, so the implementation consults `core.hooksPath` first and falls back to `--git-path`. This closes the qualification rather than leaving it latent; it is still resolution through Git, as required behavior 1 asks, and no scope was added.
...
Codex: assess Unit 5. The installer/refresh boundary accepted in Unit 4 is implemented with the failing-before/passing-after case, the collision suite, path and failure behavior, and mutation evidence that the suite can fail. Two judgments are yours: whether consulting `core.hooksPath` before `--git-path` is the right closure of the claim-(3) qualification or an over-reach on the brief's "resolve through Git"; and whether health validation now opens as the next unit or the task closes with it deferred.
