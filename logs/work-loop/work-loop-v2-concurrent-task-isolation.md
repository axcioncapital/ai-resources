...
The authorised operation does not exist in the repository as it now stands. Canonical `main` (`ef78972a`) is not an ancestor of candidate tip `94807fde`; the two lines diverged at `0ccf397c`, with 18 commits on `main` and 9 on the candidate since. The brief authorises a clean fast-forward only and explicitly withholds merge, rebase, cherry-pick and conflict resolution, so there is no move available to Claude here.
