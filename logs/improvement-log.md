...
- **Status:** logged (pending)
- **Category:** command/skill — research-prompt authoring contract; adjacent to the "instrument is not neutral" family (`ai-resources/logs/improvement-log.md:43`) but a distinct shape: there the verification *scope* was wrong by choice, here the scope is correct per its own contract and the defect lives in the seam between two artifacts it was never asked to compare.
- **Severity:** medium-high. **Stated transparently because the artifact harm alone would not justify it:** the concrete defect found was cosmetic (no claim rested on it, no gate moved). The severity comes from the control failure and the live deadline — (a) a per-question verification pass **cannot** detect this class by construction, so the control designed to catch Pack defects is blind to it; (b) `technical-engineering-services` (F6+F7) is cleared to launch and **is itself a two-question session**, so the next occurrence is scheduled, not hypothetical. Downgrade to `medium` if the F6+F7 prompt is authored per-file by hand instead.

**What happened (`industrial-software` unit, found at Stage-3 `/run-cluster`, 2026-07-30).** `research-prompt-creator` element 8 (`ai-resources/skills/research-prompt-creator/SKILL.md:110`) defines a report-level output field as **session-scoped** — "a single report-level output field … not a per-directive field." That is correct guidance when one session produces one artifact. The `industrial-software` project SOP requires the opposite shape downstream: **one Pack per question** (`projects/axcion-sector-intelligence/reference/sops/research-executor-gpt.md:102`).

The unit's Q8+Q9 session prompt combined both. `projects/axcion-si-worktrees/industrial-software/execution/research-prompts/industrial-software/session-L3.md:113` required ONE report-level "Anchor statement (REQUIRED, both anchors)" naming Anchor 1 (Q8-A03) and Anchor 2 (Q9-A01); the same file's `:129` then instructed a split write to **two** Pack files, **without stating which anchor belonged in which file**. The executor wrote the whole two-anchor field verbatim into both files — a faithful response to an ambiguous instruction, not an executor error and not a Pack-quality defect. One session-scoped field, two per-question files, **two orphan halves**.

**Why the verification control could not see it.** Step 2.4 flagged Q8 and re-verified it at Step 2.4b **in isolation, with the verifier barred from the prompts and prior reports** — deliberate context isolation, and it worked: it caught and removed Q8's orphan half (the Itron anchor; its checkpoint records "the Itron orphan anchor is gone"). But a per-question verifier reading Q9 alone sees a plausible, well-formed anchor field. **The defect is only visible by diffing the twin, which no step was scoped to do.** Neither the isolation nor the per-question scope was wrong; the seam between them was unowned.

**The generalisable rule (this is the durable part).** **When a verification pass is scoped to one artifact of a pair produced together, the unchecked twin inherits the same defect class and must be checked explicitly.** This is now the fourth instance in this one unit of a gate clearing its stated target while the defect landed just outside the checked frame — after (1) the Step-2.1b QC that checked prompts rather than emitted Packs, (2) the §5 Evidence-Pack mapping that specified deriving a value the artifact already carried, and (3) the Step-2.4b repair that added evidence without re-checking the rows recording its absence. Four instances, one shape.

- **Proposal.** Two edits, one preventer and one catcher — worth doing both, since the preventer only helps prompts authored after it lands and several units' prompts already exist.
  1. **Preventer** — `research-prompt-creator` element 8: state that report-level fields are session-scoped **by default**, and that **if the session writes one artifact per question, the prompt must either (a) name the destination file for each report-level field, or (b) redefine the field as per-question.** A session-scoped field plus a split-file write is an incomplete instruction, not a stylistic choice.
  2. **Catcher** — `research-prompt-qc` Dimension 5 (Format and Operational Compliance, `ai-resources/skills/research-prompt-qc/SKILL.md:83`): add a check that fires when a prompt declares report-level output fields **and** instructs a per-question or split-file write, and verifies each field's destination is stated. Cheap, mechanical, and it would have caught this at Step 2.1b before any research ran.
