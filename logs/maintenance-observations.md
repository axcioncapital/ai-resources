...
> Archive: [maintenance-observations-archive-2026-05.md](maintenance-observations-archive-2026-05.md)
...
## 2026-07-03 — S7 (parallel small-fix sweep)

### Maintenance debt (SO-flagged, 2026-07-03 batched risk-check second opinion)
- **Two unmanaged forks remain after this sweep.** (1) `projects/axcion-ai-system-owner/.claude/commands/consult.md` is a real-file older structural fork of the canonical command (lacks Step 0 read-first gate + Step 5a existence check); S7 pair-applied only the new Step 3.5 and marked the sync scope in-file. (2) `projects/positioning-research/.claude/hooks/friction-log-auto.sh` is a copy (down-ported byte-identical today, commit `d931d29`) — copies drift again unless re-synced or symlinked. Neither fork has an owner or re-sync trigger; weigh at a Friday cadence whether to symlink, manifest-localize, or accept-with-trigger.

### Open follow-up
- `check-foreign-staging.sh` bare-commit flag (warn when a pathspec-less `git commit` runs while a foreign per-id marker is live, even if all extra staged files are exempt) — the un-fixed half of the 2026-07-03 `9660bf2` friction entry; the documented-default half landed in `docs/commit-discipline.md` (commit `af89a07`).
