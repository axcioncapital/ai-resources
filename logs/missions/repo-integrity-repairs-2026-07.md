...
- [ ] **4. [BROKEN] The append-direction warning exists at one write site out of four.** `wrap-session.md:123` warns for `session-notes.md`; `:132` writes `decisions.md` with no warning, and `check-archive.sh:20` confirms `decisions.md` is bottom-ordered identically. The workspace-root mirror has no guard for **either** file. **Breaks:** a prepended entry is read as oldest and archived — already caused one mis-ordered commit.
  - **STILL OPEN 2026-07-25 (S1-940) — the shipped remedy is real and arguably better, but it is not what assertion 3 asks for, and assertion 6 (`:63`) forbids closing a multi-clause claim on a partial check.** Assertion 3 (`:60`) demands the append-direction warning *at each write site, in both copies, verified by a count across both*. Counted: canonical `wrap-session.md` = **1** (`:123`, `session-notes.md` only); its `decisions.md` write site at `:132` still carries none; workspace-root mirror = **0** for both files. That is **1 of 4 write sites** — the exact state the thread describes, unchanged. What `3878b4d`/`78ecf21` actually delivered is a *mechanical* guard: `check-append-order.sh`, live in the installed `.git/hooks/pre-commit` and covering `session-notes.md` and `decisions.md` at the commit boundary. **Recommended disposition — `/mission update`, not a tick:** a commit-boundary guard that cannot be forgotten is a stronger control than prose a writer must remember, so the assertion is the thing that is now mis-specified. Revise assertion 3 to require the guard (plus its regression tests) and re-judge; do not tick this thread against the assertion as currently worded.
