...
- [ ] Decide Group 3 disposition with Patrik: delete the two inert `Edit()`/`Write()` lines in `ai-resources/.claude/settings.json` (lean) vs rewrite to a real `//**` grant; same call for the workspace-root `settings.json` (local-only, non-shared). **Heads-up SENT to Patrik 2026-06-27 (Option B, repo-delivered) — awaiting his reply on intent for the two lines.** On reply: drop the pair at `ai-resources/.claude/settings.json:20-21` + the equivalent with Daniel's path at workspace-root `.claude/settings.json:41-42` (same call).
...
- [ ] **Retrofit already-deployed projects (NEW open thread, 2026-06-27).** The generator + template fixes prevent the defect on *fresh* deploys, but `/sync-workflow` does NOT re-run the grant step (it excludes both settings files), so any project deployed before these fixes may still carry a tracked `additionalDirectories`. `/permission-sweep` Rule 8 will now flag + relocate these as it sweeps. A deliberate sweep of existing projects is the remaining root-cause cleanup (lower urgency — these are ADVISORY, the path is correct on the deploying machine).
