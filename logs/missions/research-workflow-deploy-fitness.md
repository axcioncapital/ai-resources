...
- [x] **2 · Placeholder handling.** A fresh `/deploy-workflow` run fills every deploy-time placeholder (including `{{CONFIDENTIAL_IDENTIFIER_N}}`) and **no** others; every `*.template.md` file and every unused optional component survives byte-identical; the leftover-placeholder validation passes and never prompts for a template-internal value. — **PASSED, S13 (2026-07-13), by execution** against a scratch fixture on a **space-containing path** (mirroring the real deploy path `…/Claude Code/Axcion AI Repo/…`). **PRE-FIX: FAILS** — zero replacements, exit 1, `sed` error on a truncated path. **POST-FIX: PASSES** — all 26 Class-A placeholders filled including both `{{CONFIDENTIAL_IDENTIFIER_1/2}}` (which the old regex was structurally incapable of matching); all six `*.template.md` **byte-identical** (`diff -q`); the unused optional component (`produce-architecture.md`, 2 `{{PART_*}}` tokens) and the 3 Class-C notation tokens preserved; registry-scoped validation clean. The test is **falsifiable** — red before, green after. Contrast thread 1, whose frozen test was green both ways.
...
- [x] **2 — Deployment placeholder handling** (was D-3′) — **DONE, S13 (2026-07-13).** In `deploy-workflow.md` Steps 5–7: fill only immediate deploy-time placeholders (explicit list, including `{{CONFIDENTIAL_IDENTIFIER_N}}`, which today's narrow `{{[A-Z_]*}}` pattern misses); **preserve** placeholders inside deferred `*.template.md` files and inside unused optional components; and validate only those placeholders that must be resolved at deployment. Do **not** widen the discovery regex.

  > ### ⚠ RECLASSIFIED — thread 2 was NOT a demonstrated deployment blocker either. That is now 2 for 2.
  >
  > The S11 correction predicted this: thread 2's "demonstrated blocker" label came from the same audit reasoning that got thread 1 wrong, and was flagged **UNVERIFIED**. Executed in S13, it does **not** block — for the same structural reason. **The mission now has ZERO demonstrated deployment blockers.**
  >
  > **Why it never blocked.** Both live projects (`research-pe-regime-shift-advisory-gap`, `positioning-research`) were deployed from this template and carry **no genuinely-wrong unfilled deploy-time placeholder** — both `CLAUDE.md` files are fully resolved, *including* `{{CONFIDENTIAL_IDENTIFIER_1/2}}`, the very placeholders the broken regex is structurally incapable of matching. The apparent leftovers are all benign: audit/planning docs that merely *quote* placeholder names in prose, `quality-standards.md`'s Country-table notation, and — in research-pe — the four `{{PART_*_DIR}}` tokens in `produce-architecture.md`, which `SETUP.md:193–196` explicitly documents as *"only needed if the project uses `/produce-architecture`"*. That project has no `parts/` directory. **Correctly preserved, not broken.**
  >
  > The deploying agent **read** the instruction "replace the placeholders" and did it with its own tools — routing around a `sed` command that would have failed anyway. Same mechanism as thread 1: the declared contract was inert prose; the live route worked regardless.
  >
  > **The deployment gate itself should now be re-examined** (operator decision, raised S13). If zero of eight threads actually block deployment, the "fix canonical before deploying" premise does not rest on demonstrated blockers, and pilot 1 may be able to deploy sooner than this plan assumes. Threads 3–8 remain worth doing on their own merits — they were never claimed as blockers.

  **What the code actually did — three defects, all found by EXECUTION, none by any file-level audit:**
  - **Step 7 was DEAD CODE in this workspace.** `find … | xargs sed` word-splits on whitespace, and **every real deploy path contains a space** (`…/Claude Code/Axcion AI Repo/…`). Run on a real-shaped path it made **zero replacements** and exited 1 with `sed: …/scratchpad/Claude: No such file or directory`. It has never worked as written. *(Not in the audit. Not in the context pack. Found by reading the code, confirmed by running it.)*
  - **…and DESTRUCTIVE on any path without a space.** The same command on a clean path filled correctly **and mutated 5 of the 6 `*.template.md` files** (hash-verified). The step was either dead or destructive, with nothing in between.
  - **The leftover-placeholder assertion could never pass.** Step 7's verify and Step 11 item 1 both asserted zero `{{` anywhere — but 94 Class-D placeholders live inside the preserved templates and 3 Class-C notation tokens in `quality-standards.md`. A **correct** deploy failed that check by ~97 counts. A gate that always cries wolf gets ignored, which is precisely how a real leftover would have slipped through unnoticed.

  **What shipped:**
  - **Step 5 — the placeholder set is now DECLARED, not discovered.** A four-class registry: **A** required (26), **B** conditional (4 — parts-model only), **C** never-fill notation (3), **D** template-internal (94). Step 5a defines *fill scope* (excludes the six `*.template.md`, `SETUP.md`, and `produce-architecture.md` when the parts model is unused). The regex is **not widened** — it is demoted from authority to a **Step 5d drift cross-check** that **stops the deploy** on any unregistered placeholder. Guard proven falsifiable: a planted token was caught.
  - **Step 7 —** `-print0 | xargs -0` and `\( … \)` grouping (both load-bearing, both commented as such); template/SETUP exclusions; a per-project scope-list path (`/tmp/deploy-fill-scope-{PROJECT_NAME}.list`) so two concurrent deploys cannot collide.
  - **Step 7 verify + Step 11.1 —** now assert zero **registry** placeholders in **fill scope**, plus a `diff -r` **byte-identity** check on the six templates. The old "no `{{` anywhere" assertion is explicitly forbidden, with the reason stated inline.
  - **`SETUP.md` —** the Placeholder Reference table listed **8** placeholders and omitted **all 13 Project Config fields and both `CONFIDENTIAL_IDENTIFIER` fields**. An operator following it by hand would have missed 15 required values. Completed to all 34, split by class, and bound to the Step 5b registry by a stated **lockstep contract** — enforced, not merely asserted: Step 5d now diffs SETUP.md's names against the registry and stops on drift. (This is the thread-1 failure class — a declared contract silently diverging from reality — so it gets a guard, not a promise.)

  **Gates:** `/risk-check` → **GO** (`audits/risk-checks/2026-07-13-deploy-workflow-placeholder-registry-thread-2.md`). 9 consumers found, 0 must-change; one Medium (hidden coupling). The reviewer independently re-derived the placeholder math from the files (128 = 33 fill-scope + 94 template-only + 1 SETUP-only) and raised two hardening items — **both applied before commit**: the SETUP.md mirror check (above) and the per-project scope-list path.

  **Verification:** the acceptance test above, **executed** — red before, green after. Not a code read.
...
- [x] **5 — Clarify the existing evidence rules** (was C-3) — **DONE, S1 (2026-07-14).** No new permission class, as mandated. But the thread turned out to be far more than a wording fix, and **its stated premise was wrong for the third consecutive thread.**

  > ### ⚠ THE STATED PREMISE DID NOT SURVIVE A READ — 3 for 3. But the area was broken, worse than described.
  >
  > **The threshold hole S11 routed here does not exist.** S11's finding read: *"a claim with 2 sources in 1 class matches no class: `SUPPORTED` needs ≥3 sources/≥2 classes; `PROXY-SUPPORTED` needs 2 across ≥2 classes or 3+ in one class."* **Those thresholds appear in no file** — not the canonical chassis, not `claim-permission.template.md`, not either live project's `quality-standards.md` (all three grepped). They came from S11's throwaway test fixture. The canonical `SUPPORTED` row reads *"Direct evidence, ≥2 source channels, in-scope."*
  >
  > **The self-contradiction (b) was also not one.** `claim-permission-gate`'s Output schema hard-codes the four class names while Behavior step 2 calls the project's `quality-standards.md` authoritative — but the chassis's own **Canonical-ordering rule already fixes the class *names* globally** and makes only the *thresholds* project-fillable. Hard-coding the names is therefore **correct**. **Arbitrated, not "fixed":** names are canonical-global; the Conditions column is chassis-owned (previously undefined — that undefinedness was the real load-bearing ambiguity); only per-claim-type thresholds are project-fillable. Stated explicitly in the chassis now.

  **What was ACTUALLY broken — found by execution, and none of it was in the audit:**
  - **The four classes were cut on MIXED AXES** — two by evidence *quantity*, one by evidence *type* (proxy), one by *rhetorical role* (illustration). A mixed-axis cut cannot partition. It demonstrably did not: a **GAP** (one direct in-scope source that is not a named example matched *no* class — it failed `SUPPORTED` on count, wasn't proxy, wasn't a named example, wasn't zero-evidence) and an **OVERLAP** (a 2-role pattern claim matched `SUPPORTED` *and* `ILLUSTRATIVE-ONLY` at once, with no stated tie-break — *"the class table is a flat list, not a cascade"*).
  - **A SECOND overlap:** `NOT-SUPPORTED`'s conditions carried an `OR … all-source-classes-exhausted per Cond. 3/4` clause that was **not role-gated** — so a claim with ≥2 *proxy* roles whose subtask also exhausted its source classes matched `PROXY-SUPPORTED` **and** `NOT-SUPPORTED`.
  - **`ILLUSTRATIVE-ONLY` was structurally UNREACHABLE.** § Research Stop Conditions' reciprocal rule downgrades to `NOT-SUPPORTED` any claim whose subtask met no stop condition — and **no stop condition can be met by a subtask that found exactly one source** (Cond. 1 needs two; Cond. 2 needs one *plus three named examples*; Cond. 3/4 require that *nothing* was found). So every single-source claim was a "process failure." The class survived only because **no skill ever implemented the reciprocal rule** — an unenforced rule masking a contradiction, not a safeguard.
  - **`NOT-SUPPORTED` was carrying three unrelated meanings at once:** *we found nothing*, *the negative is true*, and *the researcher didn't finish*. That is premise (a)'s conflation, worse than stated.

  **What shipped (4 canonical files):**
  - **Classes re-cut onto ONE ordered axis** — independent evidentiary roles (0 → 1 → ≥2 proxy → ≥2 direct), then fit. Mutually exclusive and jointly exhaustive **by construction**. No class renamed, none added.
  - **Ceilings** — the `<3 same-pattern instances` and country-coverage conditions moved **out of the class conditions** and became **caps applied after** the evidence-graded class, reusing the already-risk-checked risk-tier ceiling mechanism. This gave the orphan `illustrative`/`directional`/`pattern-candidate` ladder a home for the first time: it is a **claim-scope** ladder, not a class. **Generalization ceiling** fires only when a claim generalizes AND lacks a warrant (**≥3 instances OR ≥1 population-level source**) — the **population-level exemption** is what lets an evidenced negative (zero positive instances *by construction*) survive.
  - **`NOT-SUPPORTED` = zero roles, and nothing else.** Provenance (a `NO-EVIDENCE` tag, a Cond. 3/4 closure) is a *route* to zero roles, never an independent trigger — *"count the roles; if the count is not zero, the claim is not `NOT-SUPPORTED`, however its subtask closed."*
  - **NEW § Evidenced Negatives vs Absence of Evidence** — an evidenced negative grades on the ordinary ladder and **can reach `SUPPORTED`**; a failed search is `NOT-SUPPORTED` and licenses **neither the claim nor its negation**; incomplete research is a *process* penalty and must be recorded as one.
  - **NEW Stop Condition 5** (max passes exhausted → confirmed evidence scarcity) — restores `ILLUSTRATIVE-ONLY` to reachability. The reciprocal rule is reclassified as a **process ceiling** (declared, and honestly disclosed as **not implementable** — the memo carries no stop-condition field).
  - **NEW upstream obligation** on `cluster-memo-refiner` Check 9: every generalizing finding records `instances: {count}` **or** `instances: n/a (population-level — {named source})`. A bare `n/a` on a generalizing claim is a memo defect the gate caps + flags.
  - **NEW chassis-version marker + hard-exit pre-flight gate** in both skills (the `/risk-check` mitigation — see below).
  - **Lockstep contract** binding the chassis + `claim-permission-gate` + `cluster-memo-refiner` Check 9 + `section-directive-drafter` + **every project's own chassis copy**.

  **Verification — BY EXECUTION, per the mission's standing rule.** Four blind adjudication runs (fresh Opus agents dispatched *as* the skill, never told the expected answers) against a fixture of 5 adversarial claims. **OLD rules: `GAPS 1, OVERLAPS 1`** — and the agent independently diagnosed the mixed-axis root cause unprompted (*"channels and instances are different axes; the class table treats them as if they were one"*). **NEW rules, final run: `GAPS 0, OVERLAPS 0, UNDETERMINED 0`.** Three rounds of correction in between — each run found real defects in the *previous* fix, including two I had introduced myself. Red before, green after.

  **Gates:** `/risk-check` → **RECONSIDER**, then **PROCEED-WITH-CAUTION** on re-fire (`audits/risk-checks/2026-07-14-…-2.md`). The RECONSIDER was **correct and caught a live hazard I had not seen**: the canonical **skills are symlinked** into the two live projects, but each project holds a **real local copy of the chassis** — so a merge updates the consumers and leaves the rules stale, and every existing pre-flight check is a *heading-presence* check that an old chassis passes. It would have **silently misadjudicated**. Closed by the chassis-version hard-exit gate. The reviewer also found a **4th consumer I missed** (`section-directive-drafter`).

  **⚠ OPEN — required mitigation, deferred to operator (mandate scope bound: canonical files only).** Neither live project's chassis is back-ported. Both are protected by the hard-exit gate and both have `.claim-permission-gate.done` sentinels present for §1.1 (so the gate does not re-run today), but **the back-port is required before either project processes a new section or anyone deletes a sentinel.** Logged as an OPEN action in **both projects' own `logs/decisions.md`**.
...
### Findings surfaced in S14 (thread 5) — routed, not fixed

- **→ NEW THREAD (thread-7-shaped). The chassis's Pass-4 permission enforcement is asserted in four skills that contain no permission vocabulary at all.** `quality-standards.md § Claim-Permission Classes` states verb-list enforcement is "Cross-checked at Stage 4.3 (`chapter-prose-reviewer`)" and that `evidence-to-report-writer` "may NOT pair `establishes`/`confirms`/`shows` with a PROXY-SUPPORTED or ILLUSTRATIVE-ONLY claim"; § No-Orphan-Citation Enforcement names `citation-converter`. **Grep finds ZERO permission-class vocabulary in `evidence-to-report-writer`, `chapter-prose-reviewer`, `citation-converter`, and `cluster-synthesis-drafter`** (surfaced by the context-discovery engine, S14; **independently grep-verified by the S14 `/risk-check` reviewer**). So the classes are assigned with care at Pass 3 and then, at Stage 4.3, **enforced by nothing** — the verb-list is normative in name only. Same failure class as threads 1 and 2: a declared contract no runtime consumer honours. Same shape as thread 7 and should probably merge into it.

  > **⚠ CORRECTION to this finding's first draft (S14, after the `/risk-check` consumer inventory).** The claim "enforced nowhere" was **too strong** and is now narrowed. `section-directive-drafter` **does** carry the permission classes into per-finding prose constraints (its Permission-Class Constraints table, `:201–206`) — it is a real, live consumer that the first draft of this finding missed. **The correct statement:** the classes ARE converted into prose constraints at directive time (Stage 3, by `section-directive-drafter`), and are then enforced by **nothing** at Stage 4.3. So the control exists but is *upstream-only and advisory* — a directive instructs, and no gate checks that the instruction was followed. That is a materially different (and smaller) hole than "enforced nowhere", and the fix is correspondingly narrower. S14 annotated `section-directive-drafter` to say its constraints are the **real** control rather than a preliminary flag. **The gap itself is still not fixed.** Verify by execution before sizing.
- **→ thread 5 follow-on / deferred cleanups. `cluster-memo-refiner` carries project-specific language inside a canonical skill.** Check 8/9 reference `Sweden / Norway / Finland`, `pan-Nordic`, `in-lens`, `above-lens`, and named vendors (`KPMG`, `Bain/Preqin`) as if they were canonical. S14 de-leaked only the lines it rewrote (the Check 9 class conditions); **the rest remains.** Same defect class as RR-02 (real names in shared skill examples). Bounded, mechanical, low-risk.

### At deployment

> **⚠ GATE RE-DECIDED — 2026-07-18 (S3-919), per the operator-approved friday-act deploy-gate-decision plan.** The "after all eight land" premise is retired: threads 1, 2, 5 (the only ones ever classified blockers) were each falsified by execution, and the mission records zero demonstrated blockers. **Deployment may proceed now** in a dedicated session against the current canonical template; threads 3, 4, 6, 7, 8 are **post-deployment improvements** (thread 3 should land before any deploy on a *different machine* — its defects are portability-class and masked on this one). Conditions and full rationale: `logs/decisions.md` 2026-07-18 (S3-919). The safeguards checklist below applies unchanged at deploy time.
...
- [ ] **F-13 — doc/wiring drift bundle** (minus row 8, which thread 7 now closes by implementation): `project-config-schema.md` row 12 names `produce-knowledge-file` as the `Delivery vault` consumer, which it never reads; `known-limits.md` is hard-class to `run-cluster` and soft-class to `run-sufficiency`; `qc-gate.md` routes to a non-existent `/run-final`; ~~`shared-manifest.json` omits three real template-local commands (`consult`, `run-sufficiency`, `session-plan`), so `auto-sync-shared.sh` cannot restore them if deleted~~ **← CORRECTED 2026-07-14 (S7), by execution. The audit's F-13(d) is FALSE as written and this list inherited it verbatim.** Only **`run-sufficiency.md`** is a real template-local command (142 lines). `consult.md` and `session-plan.md` are **symlinks out of the template** (→ workspace-root and → `ai-resources/.claude/commands/` respectively), so they are not template-local content and `auto-sync-shared.sh` has nothing to restore. Verified with `[ -L ]`, not `[ -f ]` — **`-f` follows symlinks and cannot see this distinction**, which is how the error survived both the audit and its first QC. Scope the manifest fix to `run-sufficiency` alone; the template-local `knowledge-file-producer` has its `model:`/`effort:` frontmatter stripped and so dispatches QC unpinned.
