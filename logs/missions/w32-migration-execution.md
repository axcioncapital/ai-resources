...
- [ ] **R3 Pass 2 — BLOCKED (gate closed 2026-07-12 S4). Do not start; the prerequisite below is not met.** The cut (`Files Created` / `Files Modified` / `Decisions Made` → `files_changed` / `decisions_refs`, note 8 → 5) + repointing `session-feedback-collector`'s note-grep signals at the manifest.
  ...
  - **Reopen only when:** (1) `wrap-session` writes `decisions_refs` at close (both paired copies), and (2) that is proven on **2+ ordinary wraps measured by payload** (`decisions_refs` non-empty whenever the session made decisions) — **never again by `stop_reason`**. Tracked: `logs/improvement-log.md` 2026-07-12; verdict: `logs/decisions.md` 2026-07-12 (S4).
  - **Method lesson:** a gate must test the property the downstream change *depends on*, not a proxy that correlates with it.
