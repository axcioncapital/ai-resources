...
- [ ] **R3 Pass 2 — NARROWED + P2 CLOSED (2026-07-13 S1); still not shipped, now waiting only on P1 evidence.** The cut is now **2 sections**, not 3: `Files Created` / `Files Modified` → `files_changed`, note **8 → 6** (root mirror 7 → 6). **`Decisions Made` is retained** — see P2 below. Plus repointing `session-feedback-collector`'s note-grep signals at the manifest.
  ...
  - **Reopen only when BOTH prerequisites below are met.** **P2 is now closed (2026-07-13 S1); P1 is closed but its evidence is thin** — so Pass 2 remains unshipped, waiting only on 1–2 further *ordinary* wraps producing a non-empty `decisions_refs`. This is the last thing standing in front of it. *(The original one-condition framing of this gate was itself incomplete — see the S5 method lesson; and the "one rule in one place" framing of P2 was ALSO incomplete — see the S1 method lesson.)*
