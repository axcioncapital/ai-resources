# Stage Instructions — {{PROJECT_TITLE}}

> **When to read this file:** At the start of any stage, or when a slash command triggers a stage transition. Not needed for every turn.

## Sequence Constraints

{{SECTION_SEQUENCE}}

<!-- Replace with your project's section ordering and co-dependency rules. Example:
1. Working Hypotheses before any Part 1 research
2. Part 1 sections in order: 1.1 → 1.2 → 1.3 → 1.4
3. Part 1 complete before Part 2 begins
-->

## Stage-Entry Reference-File Completeness Gate

> **When this fires:** at the *entry* of each stage that consumes a per-project reference file (Pass 3 → `run-sufficiency`; Stage 4–5 → `run-report` / `produce-*`). It is a present-**AND**-filled check, not present-only: a file that exists but still carries its template placeholders fails the gate. The contract list (which file, consuming stage, hard-vs-soft class) is `docs/required-reference-files.md` § Stage-entry reference files.

Two failure classes, handled differently — never a silent block, never a silent default:

- **Hard blocker** (e.g. `reference/stage-5-paths.md`): if absent OR unfilled (path-roots block unparsed / placeholder, or `Mode:` mismatched), **halt at stage entry** with a remediation prompt naming the file and its template. A wrong path mis-routes every downstream output, so fail loud.
- **Soft fallback** (e.g. `reference/claim-permission.md`): if absent OR unfilled (per-claim-type threshold rows still placeholder), **proceed in an explicitly-disclosed GENERIC-BAR regime** — one generic bar for all claim types — and emit a hard log line at stage entry recording that no per-type calibration is active. Disclosed, not silent; the operator sees the regime is degraded.

The check distinguishes *shape-present* (heading exists) from *values-present* (rows filled). Shape-only is treated as unfilled. Forward-only: applies at the entry of stages not yet run; does not retro-fire on completed stages.

## Stage 1: Preparation

- Read task plan draft (loaded via @ reference)
- Apply `task-plan-creator` logic to produce `/preparation/task-plans/{section}-task-plan-v1.md`
- GATE: Operator reviews Task Plan.
- After Task Plan approval: update the Project Context section of CLAUDE.md with the approved Task Plan's objective, scope, constraints, and audience.
- Apply `research-plan-creator` logic to produce research plan in `/preparation/research-plans/`
- QC Research Plan (qc-gate subagent, 6 dimensions: completeness, cluster mapping, researchability, anti-overlap, answer-spec readiness, contract conformance). Write verdict to `/preparation/checkpoints/{section}-step-3b-checkpoint.md`.
- GATE: Operator reviews Research Plan + QC verdict and approves before proceeding to answer specs.
- Researchability Triage (`run-preparation` Step 3c — inline, runs on the Step 3b-APPROVED plan): score each RQ against `reference/known-limits.md` register + `reference/source-map.md` pre-answers + Project Config `Source-availability:` posture; classify into four routes (web-answerable / proxy-only / structurally-unavailable / internal-import). Gate-reopener-eligible RQs classified as structurally-unavailable/proxy-only route to the Gate-Reopener register as proposals (not silently dropped). Write triage table to `preparation/checkpoints/{section}-step-3c-triage.md`. PAUSE for operator review at the Stage-1 gate; operator may re-route any RQ. Only web-answerable and proxy-only proceed to answer-spec generation. Forward-only: do not retro-apply to sections already past Stage 1.
- Apply `answer-spec-generator` logic to produce answer specs in `/preparation/answer-specs/{section}/`
- Run `answer-spec-qc` logic. Write verdicts to `/preparation/answer-specs/{section}/answer-specs-qc.md`
- GATE: Do not proceed to Stage 2 until operator confirms answer specs are approved.

## Stage 2: Execution (Four-Pass Model — Pass 1 + Pass 2)

> **Architecture.** Stage 2 implements the first two passes of the four-pass research model. Pass 1 (Source Discovery) ends with research prompts that target the strongest available evidence class per claim. Pass 2 (Evidence Extraction) ends with extracts that are facts-only — no synthesis yet. The strict principle: *the agent that finds evidence is not the agent that judges its sufficiency.* Pass 3 (Stage 3) does the judging.

### Pass 1 — Source Discovery

- **Step 2.0 — Create Execution Manifest [Claude Project]:** Read the Project Config `Evidence executor`, `Evidence executor capabilities`, and `Supplementary lead provider` fields plus the approved Research Plan and Answer Specs. Apply `execution-manifest-creator` to classify execution role and search mode, verify that the project-configured evidence executor satisfies every required capability, keep supplementary leads separate from evidence of record, design session groupings, and plan execution waves. If no executor qualifies, halt with the failed controls named; never silently substitute. Write eligible output to `/execution/manifest/{section}/{section}-execution-manifest.md`. GATE: Operator reviews session architecture before proceeding.
- **Step 2.0b — Source-Class Mapping [Claude Project]:** Apply `source-class-mapper` to assign each research question a primary source-class and a fallback ladder from the project's source-class hierarchy. Output: `/preparation/source-class-map/{section}-source-class-map.md`. *Invocable-but-blocked until the project provides `reference/source-class-hierarchy.md` (a project-side deliverable scheduled for a later bundle of the source-pipeline workflow fix). When the hierarchy is absent, the skill exits at pre-flight with a remediation prompt; Step 2.0b becomes a no-op until the hierarchy lands.*
- **Step 2.1 — Create Execution Prompts [Claude Project]:** Read the approved Execution Manifest, Research Plan, Answer Specs, and (when present) the source-class map from Step 2.0b. Apply `research-prompt-creator` to create platform-neutral prompts for the assigned evidence executor, following the manifest's session groupings and eligibility verdicts. Each prompt targets the question's primary source class with the fallback ladder annotated. Target 2 questions per session (1 or 3 acceptable with justification), with no hard session cap. Write each session prompt to its own file under `/execution/research-prompts/{section}/`.
- **Step 2.1b — QC Execution Prompts [delegate-qc]:** Run `research-prompt-qc` as subagent against the prompts, answer specs, and research plan. Write verdict to `/execution/checkpoints/{section}/{section}-step-2.1b-prompt-qc.md`. If APPROVED, proceed automatically. If REVISE with only Moderate findings, apply fixes and re-run once. Critical findings are classified per the skill's mechanical-vs-judgment split (`research-prompt-qc` SKILL.md § Autonomy Rules): all-mechanical → auto-fix and re-run once; judgment, ambiguous, or mixed → pause for operator. If any re-run still flags, pause for operator.
- **Transition to Step 2.2:** When Step 2.1b passes, write the execution mapping to the session notes entry (Next Steps section). Include: session table (session, questions, topic, prompt file path), wave structure with dependency notes, and per-session execution instructions. This ensures the next session can brief the operator on Step 2.2 without re-deriving the plan from the manifest and session files.
- **Step 2.2 — Execute Research Sessions [Operator]:** Operator runs every approved session in the project-configured evidence executor. Any configured supplementary provider returns leads only; the evidence executor performs its own sweep, reopens underlying sources, and logs access before use. Sessions without cross-dependencies can run simultaneously. Write raw reports to `/execution/raw-reports/{section}/{section}-session-[letter]-raw-report.md`.

> **Pass 1 principle:** AI does not answer the research question in this pass. It identifies the strongest available sources for each question and classifies them. Answering is constrained by Pass 3 permission verdicts and happens only at Pass 4.
>
> **Deferred Pass 1 items (later bundles of the source-pipeline workflow fix):** session-level country-first session structure remains deferred. *(Previously also listed here as deferred: mandatory local-language search passes, the non-substitution rule between source classes, and directive-level per-country search ordering — these have since landed in `research-prompt-creator` as § S-04, § S-02, and § S-03 respectively.)* Until session-level country-first structure lands, Step 2.1's prompt-construction continues under the current rules.

### Pass 2 — Evidence Extraction

- **Step 2.3 — Create Research Extracts [Claude Project]:** For each raw report, apply `research-extract-creator` with the report and corresponding Answer Specs. One extract per question. Extracts are facts-only — every claim carries an ID, source attribution, and (when available) source-quality fields. Write to `/execution/research-extracts/{section}/{section}-Q[N]-extract.md`.
- **Step 2.3b — Transaction Table Build [Claude Project]:** Apply `transaction-table-builder` to mine named transactions from extracts and produce the per-section transaction table at `/execution/transaction-table/{section}/{section}-transaction-table.md`. One invocation per section, between Step 2.3 (extracts) and Step 2.4 (extract verification). Downstream consumers: `cluster-memo-refiner` Check 7 (transaction-row-ID + same-pattern threshold) and `evidence-to-report-writer` named-transaction size-lens verification. *Conditionally invocable — runs when the project's `reference/source-class-hierarchy.md` exists and the project has named-transaction content in scope. Projects without named-transaction evidence (e.g., pure-thematic analyses) may skip this step; downstream consumers degrade gracefully.*
- **Step 2.4 — Review Research Extracts [delegate-qc]:** Apply `research-extract-verifier` against raw reports, extracts, and Answer Specs. Write verification to `/execution/extract-verification/{section}/{section}-extract-verification-[session-letter].md`. Verdicts: APPROVED or FLAG — RE-EXTRACT. Flagged extracts route back to Step 2.3.
- After 2 re-extraction passes, remaining failures → confirmed evidence scarcity → `/execution/scarcity-register/{section}/{section}-scarcity-register.md`
- GATE: Do not proceed to Stage 3 until all Research Extracts are APPROVED.
- **Quality log:** When all extracts are APPROVED (first pass or after re-extraction), append one row to `/logs/research-quality-log.md`. Format: `| YYYY-MM-DD | {project} | {section} | {questions} | {1st-pass approved} | {re-extracted then approved} | {scarcity} |`. Mutually exclusive categories: **1st-pass approved** = approved at first verification run; **re-extracted then approved** = required ≥1 re-extraction before approval; **scarcity** = hit ceiling without approval. Sum of three counts = Questions.

> **Pass 2 principle:** AI extracts only facts into structured artifacts (extracts, transaction table when present). No synthesis or interpretation in this pass. Synthesis is constrained by Pass 3 permission verdicts and happens only at Pass 4.

### Subworkflow 2.S — Supplementary Research (Optional)

> **Trigger:** After all extracts are APPROVED (Step 2.4 gate passed), operator reviews coverage verdicts across extracts and judges that THIN or MISSING components warrant supplementary research before entering Stage 3. Skip this subworkflow if coverage is acceptable.

> **Provider boundary:** `Supplementary lead provider: "none"` disables this path. Otherwise the configured provider returns candidate URLs only. The configured evidence executor must run its own sweep, reopen full sources, and produce an audited supplementary report before QC or merge.

- **Step 2.S0 — Extract Failed Components [Claude Project]:** Parse all approved Research Extracts and extract every component with THIN or MISSING coverage verdict. Output structured list grouped by Question ID to `/execution/supplementary/{section}/{section}-failed-components.md`. Extraction only — no query drafting. **Register-hit target-selection gate (C2):** each failed component is scored against the `reference/known-limits.md` Known-Unavailable-Evidence Register and tagged `register-hit-ceiling` (zero supplementary attempts — route straight to the scarcity register with the pass-1 proxy as ceiling), `register-hit-new-class` (one attempt, scoped to an untried source class per `source-class-hierarchy.md`), or `thin-but-closable` (standard two-pass allowance). Only the latter two proceed to Step 2.S1 query drafting. See `run-execution` Subworkflow 2.S Step 2.S0 for the full gate.
- **Step 2.S1 — Draft Supplementary Query Brief [Claude Project]:** Apply `supplementary-query-brief-drafter` with the configured supplementary lead provider, failed components, Research Extracts, and Answer Specs as inputs. Produces grouped lead-provider queries (max 12) with Section A (analysis) and Section B (paste-ready execution sheet). Write to `/execution/supplementary/{section}/{section}-query-brief-pass-[1/2].md`. GATE: Operator reviews query brief before execution.
- **Breadth-first ordering:** Pass 1 queries favor broad, high-yield source types first; Pass 2 narrows to source types Pass 1 didn't reach. See `supplementary-query-brief-drafter` SKILL.md for the pass-level breadth rule.
- **Step 2.S2 — Gather and Verify Supplementary Leads [Operator]:** Operator runs each query in the configured lead provider. Save its raw output at the historical compatibility path `/execution/supplementary/{section}/{section}-perplexity-raw-pass-[1/2].md`. The configured evidence executor then runs its own sweep, opens and logs candidate sources, and writes `/execution/supplementary/{section}/{section}-executor-verified-pass-[1/2].md`. Raw provider output never enters evidence.
- **Step 2.S3 — QC Executor-Verified Supplementary Evidence [Claude Project]:** Apply `supplementary-research-qc` against the executor-verified report, Research Extracts, and Query Brief Section A. Per-query verdicts: MERGE / SKIP / PARTIAL. Write to `/execution/supplementary/{section}/{section}-supplementary-qc-pass-[1/2].md`. GATE: Operator confirms merge summary before proceeding.
- **Step 2.S4 — Merge Supplementary Evidence [Claude Project]:** Apply `supplementary-evidence-merger` only to QC-approved, executor-verified results with a source-access log. Recalculate coverage verdicts, update syntheses, tag supplementary claims with `[SUPPLEMENTARY]`. Updated extracts replace originals in `/execution/research-extracts/{section}/`.
- **Loop ceiling (calibrated against `known-limits` — C2):** the ceiling depends on the Step 2.S0 route, not a flat two passes. `thin-but-closable` → max 2 passes (pass 2 uses the pass-2 variant of Step 2.S1, which diagnoses why pass 1 didn't close gaps and uses different search strategies); `register-hit-new-class` → 1 attempt scoped to the untried source class; `register-hit-ceiling` → 0 attempts (pass-1 proxy is the recorded ceiling). Persistent gaps after the route's allowance → confirmed evidence scarcity → `/execution/scarcity-register/{section}/{section}-scarcity-register.md`.
- Components classified as **confirmed scarcity** in pass 2 are documented in the extract's Gaps section and the scarcity register.

## Stage 3: Analysis & Gap Resolution (Four-Pass Model — Pass 3 + Pass 4)

> **Architecture.** Stage 3 implements Pass 3 (Evidence Sufficiency) and Pass 4 (Synthesis) of the four-pass research model. Pass 3 asks: *"Is the evidence sufficient to support the intended claim?"* and produces a per-cluster permission verdict (SUPPORTED / PROXY-SUPPORTED / ILLUSTRATIVE-ONLY / NOT-SUPPORTED). Pass 4 writes the chapter constrained by those verdicts — synthesis cannot promote a claim above its permission class. The Pass 3 → Pass 4 boundary is a **hard gate**: `/run-analysis` and `/run-synthesis` refuse to run without a gate-clearance file produced by `/run-sufficiency`.

> Stage 3 has 11 steps (one more than the pre-Bundle-1 layout) due to the new `/run-sufficiency` step between cluster analysis and synthesis. Split across four commands: `/run-cluster` (parallel cluster analysis), `/run-sufficiency` (Pass 3 sufficiency adjudication + gate-clearance emission), `/run-analysis` (gap resolution + directives + review + recommendations + QC), `/run-synthesis` (chapter drafting in fresh session).

### Pass 3 — Evidence Sufficiency Check

- **Step 3.1** — Read all research extracts from `/execution/research-extracts/{section}/` and scarcity register from `/execution/scarcity-register/{section}/{section}-scarcity-register.md` (if exists).
- **Step 3.2** — Apply `cluster-analysis-pass` logic for all clusters in parallel via `/run-cluster`. Main agent passes extract paths (not content) to one sub-agent per cluster; each sub-agent reads its own extracts and writes its analysis memo. Context isolation is handled within each sub-agent — no `/compact` between clusters needed in the main session.
- **Step 3.3** — Apply `cluster-memo-refiner` six-check refinement within each cluster's sub-agent (runs as part of Step 3.2). GATE: Operator reviews all refined cluster memos together before proceeding.
- **Step 3.4a — Sufficiency Adjudication [`/run-sufficiency`]:** New Opus-tier command. Orchestrates the per-cluster sufficiency check via sub-agent skills and inline rule-evaluation steps:
  - **Phase A — Claim-Permission Gate.** `claim-permission-gate` sub-agent (Opus). Assigns each claim a permission class. Output: per-cluster permission tables at `/analysis/claim-permission/{section}/`. Sentinel: `.claim-permission-gate.done`.
  - **Phase C — Country-Parity Check.** `country-parity-checker` sub-agent (Sonnet). Issues per-cluster country-parity verdicts (PARITY-OK / `{COUNTRY}`-DOMINANT / `{COUNTRY}`-THIN / MULTI-THIN / PAN-`{REGION}`-LEAKAGE / INSUFFICIENT-EVIDENCE). Output: `/analysis/country-parity/{section}/`. Sentinel: `.country-parity-checker.done`.
  - **Phase D — Stop-Conditions Check.** Inline Opus step. Applies project-defined stop conditions; verifies the scarcity register has been consulted. Output: `/analysis/stop-conditions/{section}/`. Sentinel: `.stop-conditions.done`.
  - **Phase E — Source-Conflict Resolution.** Inline Opus step. Applies the project's source-conflict procedure to any conflict triples flagged by Phases A or C. Output: `/analysis/source-conflicts/{section}/`. Sentinel: `.source-conflicts.done`.
  - **Phase F — Gate-Clearance Emission.** Inline Opus step. Computes NOT-SUPPORTED ratios per cluster and per section against the project's thresholds. Writes the gate-clearance file: `/analysis/gate-clearance/{section}/{section}-gate-clearance.md`. Sentinel: `.gate-clearance.done`. Verdict shape: `CLEARED` / `CLEARED-WITH-CAVEATS` / `BLOCKED`.
  - **Re-entry semantics.** Each phase is skip-on-sentinel-present. To force re-run of a specific phase, delete its sentinel; to force a clean run, delete all `.{phase}.done` files in `/analysis/{section}/`. The command does NOT auto-clean stale sentinels — operator responsibility.
  - **Pre-flight failure modes.** Missing refined cluster memos: exit with "run `/run-cluster` first" prompt. Missing project reference docs (source-class-hierarchy, quality-standards rule sections): exit with a remediation prompt naming the missing artifact and pointing to the project's pipeline documentation. The command never silently degrades. **`claim-permission.md` completeness (soft class, per the Stage-Entry Reference-File Completeness Gate):** Phase A checks `reference/claim-permission.md` is present AND its per-claim-type threshold rows are filled (not placeholder). If absent or shape-only, Phase A proceeds in a disclosed GENERIC-BAR regime — one generic bar for all claim types — and emits a hard log line at entry so the operator sees per-type calibration is off; it does not silently fall back to a generic default.
  - **Deferred Pass 3 phase (later bundle):** Phase B — Counter-Search (`counter-search-runner` sub-agent for disconfirming-evidence search). When Phase B is absent, `claim-permission-gate` emits an inline regime disclosure in each per-cluster permission table noting that SUPPORTED claims were not disconfirmation-tested in this run; downstream readers see the gap explicitly.
- **Step 3.4b** — Run `gap-assessment-gate` with scarcity register and the Phase A permission tables as inputs. Write report to `/analysis/gap-assessment/{section}/{section}-gap-assessment.md`.
- **Path classification:** Path A = gap requires new primary research (missing evidence categories, unasked questions, or fundamentally insufficient source material). Path B = gap can be closed with targeted factual retrieval (specific data points, statistics, or confirmatory evidence for existing analysis).
- If Path B gaps: execute Subworkflow 3.S (configured supplementary provider for leads; evidence executor for source verification). After 3.S completes: re-run `gap-assessment-gate` on affected clusters only before proceeding to Step 3.5.
- If Path A gaps: loop back to Stage 2 for affected questions only. Re-entry trigger: new APPROVED research extracts for the affected questions appear in `/execution/research-extracts/{section}/`. Resume Stage 3 from Step 3.2 for the affected clusters only. Unaffected clusters hold at their current completed state — do not re-run them. Termination: all affected questions either resolve or exhaust 2 supplementary passes and classify as scarcity.
- **Reconvergence (Path A):** Write a loop-state entry per affected question to `/analysis/checkpoints/{section}/{section}-gap-loop-state.md`, tracking: question ID, current pass count, status (looping / resolved / scarcity). Stage 3 resumes at Step 3.5 only when every entry shows resolved or scarcity. Operator confirms loop completion at the gap assessment gate below.
- GATE: Operator reviews gap assessment routing decisions.

### Subworkflow 3.S — Lightweight Gap Research (Path B)

> **Trigger:** Step 3.4 routes gaps to Path B — analytical gaps that can be closed with targeted factual retrieval through the configured supplementary lead provider, without requiring full Stage 2 research sessions. If the provider is `none`, halt this path for an operator decision; do not invent a lane.

- **Step 3.S1 — Draft Gap Queries [Claude Code]:** Read gap assessment, affected cluster memos, and Research Extracts for affected questions. For each query group, document: (a) **What Stage 2 already searched** — source types, institutions, and search angles tried by the configured evidence executor and any Stage 2 supplementary passes, including searches that returned nothing useful; (b) **What the supplementary lead provider should find that's different** — source types, data categories, or entry points Stage 2 did not target. Each query must target an absent source class, stay single-intent, include required local terminology, name known primary institutions, and carry an appropriate recency annotation. Draft at most 8 queries. Write to `/analysis/gap-supplementary/{section}-gap-queries-pass-[1/2].md`. GATE: Operator reviews queries before execution.
- **Breadth-first ordering:** Pass 1 queries favor broad, high-yield source types first; Pass 2 narrows to source types Pass 1 didn't reach. See `supplementary-query-brief-drafter` SKILL.md for the pass-level breadth rule.
- **Step 3.S2 — Gather and Verify Leads [Operator]:** Operator runs each query in the configured supplementary lead provider and saves raw output to the historical compatibility path `/analysis/gap-supplementary/{section}-perplexity-raw-pass-[1/2].md`. The configured evidence executor then performs its own sweep, opens and logs candidate sources, and writes `/analysis/gap-supplementary/{section}-executor-verified-pass-[1/2].md`.
- **Step 3.S3 — QC and Merge Verified Results [Claude Code]:** Review only the evidence executor's verified report against the target gaps; raw provider output is never evidence. Per-query verdicts: MERGE / SKIP / PARTIAL. For MERGE and PARTIAL results: (1) write a lightweight supplementary extract with Claim IDs, source attribution, strength ratings, and executor audit locators to `/analysis/gap-supplementary/{section}-gap-extract-pass-[1/2].md`; (2) integrate the ID-bearing claims into affected cluster memos, tagged `[GAP-FILL]`. Write QC report to `/analysis/gap-supplementary/{section}-gap-qc-pass-[1/2].md`.
- **Loop ceiling:** Max 2 passes. Pass 2 must update the "already searched" inventory to include pass 1 queries and outcomes, then use different search strategies targeting source types not yet tried. Persistent gaps after 2 passes → confirmed evidence scarcity → `/execution/scarcity-register/{section}/{section}-scarcity-register.md`.
- After 3.S completes: return to Step 3.4 for re-assessment of affected clusters only.

> **Pass 3 principle:** A separate AI reviewer asks: "Is this enough to support the intended claim?" The answer is one of four permission classes per claim. The reviewer is not the agent that found the evidence and not the agent that will write the prose. The gate-clearance file emitted by Phase F is the load-bearing artifact: Pass 4 cannot start without it.
>
> **Deferred Pass 3 items (later bundles of the source-pipeline workflow fix):** disconfirming-evidence search (counter-search-runner, Phase B); monitoring-vs-catalyst routing of speculative claims. Until those land, Pass 3's surface is the slimmed phase set above (A → C → D → E → F).

### Pass 4 — Synthesis

> **Pass 4 pre-flight — two hard gates, not one.** Gate-clearance answers *is the evidence sufficient?*; approved judgment answers *what is the view, and did the founder approve it?* They are different questions and neither substitutes for the other.
>
> **Approved-judgment gate.** `/run-synthesis` (Step 0b), `/run-report` (Step 4.0b) and `/run-analysis` (Step 3.5e) each run `logs/scripts/check-judgment-contract.sh {base}-approved.md` and halt on any nonzero exit. Each is a **separate command the operator invokes directly in a fresh session**, so each carries its own gate — `/run-analysis`'s gate protects `/run-analysis` and nothing downstream of it. Beyond the gate, the section directives, cluster synthesis, report architecture, chapter prose and compliance QC owners each receive `{base}-approved.md` **by path** and must shape their output from its theses, provisional verdict, countercases and change conditions, recording the `Thesis N` each consequential claim or structural choice implements. Running after the gate is not consumption. **Where the approved brief conflicts with a permission table, scarcity instruction, gate-clearance caveat or operator decision, the command halts and surfaces both authorities** — judgment governs downstream work, it does not override the other controls.
>
> **Gate-clearance gate.** Every Pass 4 step that produces report-bound content (`/run-analysis`, `/run-synthesis`) reads `/analysis/gate-clearance/{section}/{section}-gate-clearance.md` at Step 0. If the file is absent, or if its verdict is `BLOCKED`, the command exits with a remediation prompt; it does not silently proceed. If the verdict is `CLEARED-WITH-CAVEATS`, the command proceeds and attaches the caveat list to the synthesis brief passed to `cluster-synthesis-drafter`. The operator may manually override a `BLOCKED` verdict by setting the gate file to `OPERATOR-OVERRIDE` with a signed rationale line; the override is auto-logged to `/logs/decisions.md`.

- **Step 3.4c — Unit Judgment Brief [`/run-analysis` Step 3.5]:** the analytical-authority seam. Gaps are resolved and no report-bound writing has started, so this is where combined research becomes a stated Axcíon view. Four moves, in order: a producer sub-agent writes `/analysis/judgment/{section}/{section}-unit-judgment-brief-proposed.md` from an **evidence bundle** and an **Axcíon context bundle** kept as separate inputs; a fresh-context reviewer that did not write it challenges the proposal, and the challenge record binds to the exact proposal bytes via `reviews_sha256:`; the route **halts unconditionally** for the founder's explicit **approve / revise / reject** — no timeout, no auto-approve, no continuation on silence; and on approval only, `logs/scripts/promote-judgment-brief.sh` re-issues the brief mechanically as `-approved.md`. A revision changes the proposal's bytes, which makes the recorded challenge stale by construction and forces a new round. A rejection is durable and creates no approved artifact. Authority rules: `docs/judgment-authority-contract.md`. Shape and authoring guidance: `reference/unit-judgment-brief.template.md`.
  - **⚠ Judgment gate (hard).** The two files number this boundary differently and both numbers are correct in their own file: this stage's **Step 3.5 (Section Directives, below)** is the command's **Step 4**, and the seam described here is the command's **Step 3.5**. The gate is the same either way — section directives do not start until `logs/scripts/check-judgment-contract.sh {base}-approved.md` exits `0`. Every other exit halts: `3` no approved brief, `4` present but not approved (still proposed, or rejected), `5` no evidence basis, `6` structural failure. Branch on the code, never on the prose, and never regenerate over a present-but-invalid authority.
- **Step 3.5** — Apply `section-directive-drafter` per cluster. MUST include scarcity register items AND the per-cluster permission tables from Pass 3 Phase A as required inputs. The directive may not instruct a NOT-SUPPORTED claim into the chapter; PROXY-SUPPORTED and ILLUSTRATIVE-ONLY claims must carry their permission-class signal into the directive's prose rules.
  - **⚠ Chassis-provenance gate (added 2026-07-14).** Every permission table consumed here must carry `chassis_version: 2026-07-14` or later in its frontmatter, with `generated_at` ≥ `chassis_version`. `section-directive-drafter` **hard-exits** otherwise: a table graded under the pre-2026-07-14 rules carries verdicts from a rule set with a demonstrated gap and overlap, and converting them into prose constraints silently licenses over-claims (verified by execution — 2 of 6 claims in a real table changed class). The same gate sits on `/run-sufficiency` **Step 0** (the re-entry path, where Phase A is skipped) and on `/run-synthesis`. **On a hard exit: back-port the chassis FIRST, then delete the sentinel, then re-run** — that order, or you land in a second hard exit with the sentinel already deleted and no revert that restores it. Full rule: `reference/quality-standards.md § Provenance is stamped on outputs, not only gated on inputs`.
- **Step 3.6** — Apply `analysis-pass-memo-review` for editorial decision surfacing. Write to `/analysis/editorial-review/{section}/{section}-memo-review.md`.
- **Step 3.6b** — Apply `editorial-recommendations-generator` to produce recommended answers for all editorial decisions. Write to `/analysis/editorial-review/{section}/{section}-memo-review-recommendations.md`. [delegate]
- **Step 3.6c** — Apply `editorial-recommendations-qc` as independent QC of recommendations. Write to `/analysis/editorial-review/{section}/{section}-qc-editorial-decisions.md`. [delegate-qc]
- **Step 3.6d** — Auto-approve editorial decisions based on QC results. [auto-delegate] Decision logic: QC AGREE → accept as-is; QC NUANCE → accept with QC adjustment applied; QC DISAGREE → PAUSE for operator. Write to `/analysis/editorial-review/{section}/{section}-editorial-decisions-approved.md`. Log to `/logs/qc-log.md`. Only pauses if QC contains DISAGREE items. Session boundary — start new session for Step 3.7.
- **Step 3.7** — Apply `cluster-synthesis-drafter` per cluster, write to `/analysis/chapters/{section}/`. Runs via `/run-synthesis` in a fresh session. The drafter is constrained by the per-cluster permission tables: a chapter may not state a NOT-SUPPORTED claim; PROXY-SUPPORTED and ILLUSTRATIVE-ONLY claims must be labeled in prose accordingly. The synthesis brief includes any gate-clearance caveats.

> **Pass 4 principle:** Only now does the AI write. The chapter is constrained by what Pass 3 permitted, not by what Pass 1 + 2 found. The mapping from evidence to permitted prose is the load-bearing artifact of the four-pass model.

## Stage 4: Report Production

- **Step 4.1** — Read all chapter drafts from `/analysis/chapters/{section}/` and scarcity register from `/execution/scarcity-register/{section}/{section}-scarcity-register.md`. Apply `research-structure-creator` to produce `/report/architecture/{section}/{section}-architecture.md`.
- For each chapter:
  - **Step 4.2** — Apply `evidence-to-report-writer`. Scarcity register is a required input — writer MUST implement editorial instructions for scarcity items. Write to `/report/chapters/{section}/{section}-chapter-NN-draft.md` (the `-draft.md` suffix is canonical per S-16 — the operator-editable working file; `evidence-to-report-writer` also emits a `§ Reviewer Findings Summary` footer carrying the top 3 priority flags from `chapter-prose-reviewer`, stripped automatically at Step 4.4b). The writer applies named-transaction size-lens verification against `/execution/transaction-table/{section}/{section}-transaction-table.md` when the table is present (S-05).
  - **Step 4.3** — Apply `chapter-prose-reviewer` to the draft. Write the review report to `/report/chapters/{section}/{section}-chapter-NN-review.md` (named to be discoverable alongside the draft for operator review).
  - **Step 4.4 (Step 4.1b per S-16, blocking gate)** — Operator review of chapter draft. Pipeline halts; operator edits the draft in-place using `<!-- improve: [idea] -->` and `<!-- KEEP -->` inline HTML-comment markers, then replies `approved` (case-insensitive whole-word, or first word of the reply). On `approved`, `/run-report` writes the operator-approval marker at `/report/chapters/{section}/{section}-chapter-NN-OPERATOR-APPROVED.md` (single-line content: `APPROVED: YYYY-MM-DD-HH-MM | <optional note>`). The marker is the canonical signal `citation-converter` reads at its Step 0a pre-flight. The literal token `approved` and the marker filename suffix `-OPERATOR-APPROVED.md` form an atomic two-end string contract — both ends edit together.
  - **Step 4.4b (Step 4.1c per S-16)** — Apply `chapter-revision-applier`. Reads the draft + markers, applies inline `<!-- improve: -->` ideas to surrounding paragraphs, preserves `<!-- KEEP -->`-marked paragraphs, strips all markers and the `§ Reviewer Findings Summary` footer, writes `/report/chapters/{section}/{section}-chapter-NN-revised.md`. The original draft is preserved unchanged for audit. Idempotent on re-run if the revised file is newer than the draft.
  - **Step 4.5** — Apply fixes from prose review per bright-line rule. After first chapter: lock style reference to `/report/style-reference/{section}/{section}-style-reference.md`.
  - **Step 4.6** — Add H3 titles.
  - **Step 4.7** — Apply `citation-converter` to the revised draft (NOT the original draft). Step 0a pre-flight verifies the operator-approval marker exists; if absent, the skill refuses to run and the pipeline halts. Write cited version and Citation Traceability Layer. After successful conversion, `/run-report` archives the marker to `.archive/{section}-chapter-NN-OPERATOR-APPROVED-{timestamp}.md` to prevent stale-marker bypass on re-runs.
- GATE: Operator reviews each chapter before proceeding (Step 4.4 is the formal gate; Step 4.7 produces the cited artifact).

## Stage 5: Final Production

> **Output paths (report-mode):** read from `reference/stage-5-paths.md` at Phase 0. Compiled sources and final deliverable → `report/compiled/{section}/`. Prose-refinement and formatting outputs → `report/produced/{section}/R{N}/`. Working files → `report/produced/{section}/R{N}/working/`. Per the Stage-Entry Reference-File Completeness Gate (hard-blocker class), `stage-5-paths.md` must be present **AND filled** — if it is absent, or present but its `## Stage 5 Path Roots` block is unparsed/placeholder, or its `Mode:` line mismatches the project's `Document model`, Stage 5 commands halt at Phase 0 with a remediation prompt. Do not proceed silently.

- **Step 5.1** — Run `document-integration-qc` on the compiled source. Write QC report to `report/compiled/{section}/{section}-R{N}-integration-qc.md`.
- **Step 5.2** — Apply fixes per bright-line rule.
- **Step 5.3** — Apply prose-refinement, decontamination, jargon-gloss, and formatting passes via `produce-prose-draft` and `produce-formatting`. Write formatted output to `report/produced/{section}/R{N}/R{N}-formatted.md`; intermediate working files to `report/produced/{section}/R{N}/working/`.
- **Step 5.4** — QC formatting against style reference.
- GATE: Operator reviews formatted draft (Step 5.4).
- **Step 5.5** — Implement fixes in three phases: (1) identify all flagged items from QC reports, (2) classify each per bright-line rule (auto-fix vs. operator approval), (3) apply approved fixes and write v2.0. Bright-line rule applies to every fix.
- **Step 5.6** — Run `citation-converter` in reconciliation mode for bibliography.
- **Step 5.7** — Merge and compile full document. Write compiled final to `report/compiled/{section}/{section}-R{N}-final-v{M}.md`.
- **Step 5.8** — Compile glossary, add front matter to the compiled final at `report/compiled/{section}/{section}-R{N}-final-v{M}.md`.
- GATE: Final operator review.

## Context Management

Pipeline commands consume large amounts of context. Three strategies keep usage sustainable:

### Strategy 1: Step Checkpoints
After each major step, write a checkpoint file capturing what was produced, key decisions, and anything the next step needs. Subsequent steps read the checkpoint instead of re-reading all source material.

- Path: `{stage-dir}/checkpoints/{section}-step-{id}-checkpoint.md`
- Content: structured summary — file paths produced, counts/inventory, decisions, flags for next step. Target: under 500 tokens.

### Strategy 2: Compact Directives
Commands include explicit `▸ /compact` markers at natural boundaries — typically after a step that loaded a skill file and wrote its output.

### Strategy 3: Sub-Agent Delegation for Heavy Steps
Self-contained steps run as sub-agents via the Agent tool. The main session never loads the skill file — it reads the inputs, passes them to the sub-agent along with the skill content, and receives a structured summary back.

Pattern for delegated steps:
1. Main agent reads the input files needed for the step.
2. Main agent reads the skill file.
3. Main agent launches sub-agent, passing: skill content, input content, output path, and task description.
4. Sub-agent executes, writes output, returns summary.
5. Main agent writes step checkpoint from summary.
6. Main agent compacts.

When a step is marked **[delegate]**, follow this pattern. When marked **[delegate-qc]**, use the qc-gate sub-agent type.

## Context Isolation Rules

- Sub-agents receive content from the main agent, not file paths. The main agent reads the file and passes the content.
- Exception: Verification Agent reads source files directly (independent derivation), but receives the main output from the main agent.
- Exception: large read-only reference files (`style-reference.md`, `context/prose-quality-standards.md`) may be passed by absolute path when the subagent is instructed to read them before applying the skill. Rationale: avoids duplicating ~1,200 lines of reference content across main-agent context and each subagent brief. Operand artifacts (the document being worked on) and skill content remain content-passed per the general rule.
- Sub-agents do not persist state between invocations. Each call is fresh. If prior results are needed, the main agent provides them explicitly.
- Sub-agents do not inherit the parent agent's session state. When launching a sub-agent, pass the working state it needs — output directory paths, files already created, stages completed — so it does not rediscover what the parent already knows.

## Citation Conversion Rule

Every cited chapter file must include a complete bibliography listing all sources cited in that chapter. Never substitute a note like "sources listed in other modules" or "no new sources introduced." Even if every source was introduced in a prior module, the bibliography must reproduce the relevant entries. Each chapter is a self-contained cited artifact.

## Bright-Line Rule

Before applying ANY fix to report prose, run three checks:
1. **Multi-paragraph scope:** Changes more than one paragraph? → PAUSE for operator approval.
2. **Analytical claim alteration:** Changes, removes, or reframes an analytical claim? → PAUSE for operator approval.
3. **Sourced statement modification:** Alters a statement attributed to a source or carrying a claim ID? → PAUSE for operator approval.

If ANY check is true, the fix MUST NOT be applied without explicit operator approval. Log to `/logs/decisions.md`.

Applies at: Step 4.2, Step 5.2, Step 5.7, and `/verify-chapter`.
