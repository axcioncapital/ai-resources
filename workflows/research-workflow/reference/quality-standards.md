...
> **Canonical-ordering rule.** This chassis is the source of truth for permission-class **names**, the permitted-prose-**verb lists**, the **class conditions** (the Conditions column below — the generic evidence bar), the **ceiling** semantics, and the **gate** semantics. Per-claim-type evidence thresholds — the project-fillable calibration *on top of* the generic bar — live in `reference/claim-permission.md` (derived from `claim-permission.template.md`). Future edits that cross this boundary in either direction require a fresh risk-aware review.
>
...
This section is the **authority**. Four other files carry copies or literals of what it defines, and they were enumerated by grep, not by assumption (a review consumer inventory, 2026-07-14, found one this contract's first draft had missed):
...
| `section-directive-drafter` | **Restates the verb lists and prose framing** per class (not the conditions); converts classes into per-finding prose constraints. **Also pins the required `chassis_version` in its chassis-provenance gate** (added 2026-07-14) | **verb lists or prose framing** change — **and, since 2026-07-14, any edit that bumps the version marker**, because its gate hard-exits on tables older than the version it pins. A conditions-only edit no longer leaves this skill untouched. |
| `/run-synthesis` (Stage 4 command) | **Reads the per-cluster permission tables and passes them to `cluster-synthesis-drafter`** to constrain what prose may state. Carries the chassis-provenance gate on those tables (added 2026-07-14) | any edit that bumps the version marker — its gate hard-exits on tables older than the version it pins. **Found by grep, 2026-07-14; it was missing from the first draft of this contract**, which named only the three skills. |
| `/run-sufficiency` **Step 0** (Pass-3 orchestrator) | Carries the chassis-provenance gate on **pre-existing** permission tables — the re-entry path where `.claim-permission-gate.done` is present, **Phase A is skipped**, and Phases D and F consume a previous run's verdicts (Phase F computes the gate-clearance verdict every Pass-4 command reads) | any edit that bumps the version marker. **Found in review, 2026-07-14 — missed by both the first draft AND the grep sweep**, because Phase A's gate looks like it covers this command and does not: on re-entry Phase A never runs. |
...
| **Each consuming project's own `.claude/commands/`** | **Real local copies of the workflow commands** — *not* symlinks (verified 2026-07-14) | **always**, for any command carrying a version-pinned gate. Same copy-distribution hazard as the chassis row above. |

> **`country-parity-checker` reads the permission tables but is deliberately NOT gated** (decided 2026-07-14). It is a required-input reader, but its own rules forbid it from judging whether a claim is supported ("Do NOT use to judge whether a claim is sufficiently supported — `claim-permission-gate`'s job"); it consumes the *claim inventory*, not the *class verdicts*, and its country-parity output does not change when a class changes. Gating it would halt a skill whose output a stale table cannot corrupt — gate ceremony without a hazard. Revisit only if it starts reading the Assigned-class column.
...
### Provenance is stamped on outputs, not only gated on inputs (added 2026-07-14)

The version gate above protects the **next** run. It says nothing about the **verdicts already on disk** — and a permission table is a set of confident class assignments that downstream stages consume as authoritative. A table graded under an old chassis is therefore the same hazard one layer down, and the gate cannot see it.

**The rule.** Every permission table carries `chassis_version:` in its frontmatter — the marker read at pre-flight, copied verbatim — written by **both** producers (`claim-permission-gate`, `cluster-memo-refiner` Check 9), and **verified by every consumer** (`section-directive-drafter` and `/run-synthesis` hard-exit on a missing or older-than-required value). Provenance travels *with the artifact*. A verdict whose rule set cannot be identified is not a verdict a downstream stage may act on.

**And the stamp must be earned, not merely present — the re-stamp invariant.** `chassis_version` is a *self-asserted* field. A check that only asks "is the field there, and is the date recent enough?" tests whether the artifact **claims** freshness, not whether its verdicts **are** fresh — and one pasted line defeats it. Every consumer therefore also enforces:

> **`generated_at` ≥ `chassis_version`.** A table cannot have been produced *before* the rules that produced it existed. If `generated_at` precedes `chassis_version`, the table was **re-stamped, not re-adjudicated** — refuse it, and tell the operator to delete the sentinel and re-run the gate rather than hand-edit the field.

This costs nothing (both fields already exist) and it is what gives the stamp meaning. It is not a hypothetical hardening: `/run-sufficiency` documents an operator path that edits permission tables **by hand**, and a refusal message that names the missing field is an invitation to paste it. **The invariant was added because an adversarial run of the gate's own first draft passed a table forged exactly that way** (2026-07-14) — the gate was written, tested red-then-green, and *then* shown to be defeatable by a one-line edit. Verification found it; reasoning had not.

**What this gate does and does not buy — stated plainly, because a gate that oversells itself is worse than none.** An adversarial run on 2026-07-14 confirmed the limit: `chassis_version` and `generated_at` are **both self-asserted fields in the file the operator edits**, so the invariant is a comparison between two values the same hand controls. Nothing binds the frontmatter to the verdict rows beneath it. Therefore:

- **It stops staleness by neglect — the demonstrated hazard, and the only one observed.** A table honestly produced under old rules and left on disk (both live projects, section 1.1) hard-exits. Nobody forged anything; the table simply predates the rules. This is the case the gate exists for and it closes it completely.
- **It stops the single-line hand-paste** — the careless route straight out of a hard-exit message.
- **It does NOT stop a determined operator.** Two edited fields pass it. That is not a defect to be patched — it is an **override**, and this workflow already has a legitimate one: `OPERATOR-OVERRIDE`, signed, with a rationale logged to `/logs/decisions.md`. **An operator who means to proceed on a stale table must take that path, not edit the frontmatter.** A silent frontmatter edit is an unlogged override wearing a compliant stamp, and no gate can distinguish it from a real re-run.

**A content hash over the verdict rows would close even that** — and was deliberately **NOT** built. Two reasons, both structural: (1) these are **model-executed skills**, not scripts — an LLM cannot compute a trustworthy digest without new shell machinery, so the "cheap" hardening is not cheap; (2) the threat model here is **operator error, not an adversary**, and the date invariant already catches every careless path. **Smallest general fix wins.** Revisit only if a forged stamp is ever observed in practice — not in anticipation of one.

> **Known limit on the acceptance test itself, recorded so no one mistakes it for coverage.** The "green" fixture used to verify this gate (`chassis_version: 2026-07-14`, `generated_at: 2026-07-14`, unchanged body) is **byte-identical below the frontmatter to the stale one.** So the test proves the gate admits a table whose frontmatter *claims* re-adjudication — it does **not** prove the gate admits a table that *was* re-adjudicated, because from the file those are the same artifact. The gate cannot see the difference, and neither can its test. Do not read the green case as stronger than it is.

**This was demonstrated by execution on 2026-07-14, not reasoned from the files** (the standing method rule for this chassis — see § Adjudication order). Re-adjudicating a real pre-2026-07-14 table (`research-pe-regime-shift-advisory-gap`, section 1.1, cluster 03) under the current rules moved **2 of 6 claims** from `PROXY-SUPPORTED` to `ILLUSTRATIVE-ONLY`, and all 6 gained a `[GENERALIZATION-CAPPED]` marker the stale table lacks entirely. A blind `section-directive-drafter` run against the stale table then confirmed the consequence: it emitted *"hedged framing required"* for both reclassified claims — **licensing two market-pattern generalizations that the current rules forbid outright**, because `PROXY-SUPPORTED` carries no generalization prohibition and only `ILLUSTRATIVE-ONLY` does. Asked how it could tell which chassis produced the table, it answered that it could not: no version field existed on either side. **The failure was silent in every stage and detectable in none.**

**The distribution topology, verified 2026-07-14 — know it before you reason about a merge.** It is mixed, and the mix is the hazard:

| Component | How it reaches a deployed project | What a canonical merge does to it |
|---|---|---|
| **Skills** (`claim-permission-gate`, `cluster-memo-refiner`, `section-directive-drafter`, `country-parity-checker`) | **Symlink** into `ai-resources/skills/` | **Lands instantly.** No project action needed. |
| **This chassis** (`reference/quality-standards.md`) | **Real local copy** | **Nothing.** Stale until back-ported. |
| **Workflow commands** (`.claude/commands/`, incl. `/run-synthesis`) | **Real local copy** | **Nothing.** Stale until back-ported. |
| **Analysis artifacts** (permission tables, directives) | Produced *in* the project, under whatever rules were live at the time | **Nothing.** They outlive every gate. |

So a canonical merge updates **exactly one** of these four rows. The other three keep whatever they had — which is why a gate on the symlinked consumer, alone, is never enough.

**The general shape, worth naming:** where a *consumer* is distributed by symlink and the *rules it depends on* travel by copy, a version gate on the consumer is necessary but not sufficient — the **artifacts produced under the old rules outlive the gate**. Stamp the producing rules' version onto the artifact, and make every consumer check it. A presence check (does the heading exist?) can never substitute for a version check, because a stale copy has all the same headings.

An edit to the Conditions column, the class names, the adjudication order, or the ceilings requires the paired edits above **and** a fresh risk-aware review, per the Canonical-ordering rule. Acceptance test for any such edit: the worked-cases table in § Adjudication order — run every case through the changed rules and show each lands in exactly one class.

> **⚠ Bumping the version marker is a FOUR-FILE obligation, and forgetting it fails SILENTLY — in the permissive direction.** The four provenance gates (`claim-permission-gate`, `cluster-memo-refiner`, `section-directive-drafter`, `/run-synthesis`, `/run-sufficiency` Step 0) each pin a **required floor** (`2026-07-14` or later), not the *current* marker. So if the chassis is bumped to a new version and those floors are **not** bumped with it, every gate still passes tables graded under the **superseded** rules — which is precisely the staleness the gates exist to catch, reintroduced by omission. **A version bump is not done until every pinned floor equals the new marker.** Grep for the pinned version across all five consumers and confirm they match before you land the bump.
