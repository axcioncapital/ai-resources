...
**Required use of approved judgment:** add a **STAGE 3 — House View fidelity** check, after Stage 2, and report it as its own findings group. This is the last independent look at the report before it is signed off, and it is the only one positioned to see what the whole Stage 5 chain did to the approved argument. Three checks: **fidelity** — the formatted report still argues the approved theses, holds the provisional verdict at its approved strength, and carries each countercase at the weight the brief gave it; **thesis trace continuity** — every `Thesis N` trace that entered Stage 5 is still present and still attached to the claim it belongs to, and no consequential claim has appeared without one; and **drift** — a claim the approved argument does not carry, a verdict sharpened past what was approved, a dropped countercase, a change condition lost in formatting, or wording that exceeds the evidence-permission class of the claims its thesis rests on. Judge all three against the approved brief and the formatted prose **only**. The Phase 2 change log and the Stage 1 fixes-applied log are the producers' accounts of what they did; they may explain a finding but may never satisfy this check, because a producer that dropped a trace will not report having dropped it. Drift and permission overreach are FAIL findings, not notes.
   - Task summary: **report-mode — execute Stage 1, then Stage 2, then STAGE 3 — House View fidelity, in that order. Section-mode is two-stage: Stage 1 then Stage 2, and STAGE 3 does not run.** **Output-to-disk pattern:** Write the full structured QC report to `{prose_output_dir_abs}/<working-dir-name>/formatting-phase-3-qc-{arg}.md`. Sections of the working file (all required): Stage 1 findings (formatting) with severity, Stage 1 fixes-applied log, Stage 1 bright-line candidates, Stage 2 findings grouped by check category (Narrative Structure / Consistency / Redundancy & Contradiction / Completeness), transition drafts (if any), **Stage 3 House View findings (report-mode) as their own group, split into fidelity / thesis-trace-continuity / drift with a severity on each, plus a distinct `stage_3_verdict` of PASS or FAIL**, and overall verdict.
   - **Return to main session (no more than 20 lines):**
     - `working_file`: absolute path to the file just written
     ...
     - **report-mode only —** `stage_3_fidelity_count`, `stage_3_trace_count`, `stage_3_drift_count`, and `stage_3_verdict`: `PASS` or `FAIL`. These are separate fields, not prose inside `verdict`: the main session routes on them at step 9, and a finding the route cannot read is a finding the route ignores. Where STAGE 3 could not be run — the approved brief was unreadable at its path — return `stage_3_verdict: FAIL` with the reason, never `PASS` and never omission.
     - `verdict`: one line. In report-mode this may not be a clean verdict while `stage_3_verdict` is `FAIL`.
9. Route on findings:
   - **`stage_3_verdict: FAIL` (report-mode) — this route does not reach handoff.** Authority drift, a lost thesis trace or evidence-permission overreach means the formatted report no longer says what the founder approved, and it is the one finding class Phase 4 must never receive as a clean result. **Halt**, present the Stage 3 findings group to the operator with the approved brief path, and log the decision to `logs/decisions.md`. Proceed to Phase 4 only after the operator has resolved every Stage 3 FAIL finding — by correcting the prose, or by an explicit decision recorded there. A Stage 3 FAIL is never cleared by the Stage 1 or Stage 2 results being clean.
   - **No SUBSTANTIVE findings in any stage that ran, and `stage_3_verdict: PASS` where report-mode ran STAGE 3:** Note results and any NON-SUBSTANTIVE items. Proceed to Phase 4 (handoff).
