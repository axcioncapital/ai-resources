...
- **`templates/`** holds canonical deployable fragments consumed at scaffold time (project settings + project AGENTS.md sections) — edit the fragment, not the consuming command. See `templates/README.md` for the consumer contract.
...
These resources operate across a multi-tool ecosystem — not just Codex. Skills may reference GPT-5 (via API/CustomGPT), Perplexity (via API), Notion, and NotebookLM. Do not design resources that assume a single-tool environment.
...
**Model defaults are prohibited** — never add a `"model"` field to any settings.json or declare a default model in any `AGENTS.md`; per-command/agent/skill `model:` frontmatter is the only permitted tiering mechanism. Full rule and rationale: workspace `AGENTS.md` → Model Tier.
...
Run `/friday-checkup` each Friday before starting next week's work. Auto-detects tier (weekly / monthly / quarterly), asks which projects are active, runs the tier's audits across `ai-resources/`, workspace root, and named projects, and writes a consolidated review-only report to `ai-resources/audits/friday-checkup-YYYY-MM-DD.md`. Full mechanics: `.Codex/commands/friday-checkup.md`.
...
Codex permission prompts (Edit / Write / Delete) are managed structurally, not reactively. Canonical shapes for all settings layers (user / workspace / ai-resources / project) live in `docs/permission-template.md`. The `/permission-sweep` command diagnoses and remediates drift across all layers in one approved pass; `/new-project` emits the canonical project template for every new project; the `check-permission-sanity.sh` SessionStart hook nudges on drift. Pairs with `/fewer-permission-prompts` (which handles empirical, transcript-driven gaps).
...
Full commit/push behavior — including the wrap-time push confirmation prompt — is canonical in workspace `AGENTS.md` → File verification and git commits.
...
- The exact active `logs/work-loop/{task-id}.md` path (if a Work Loop task is open).
- The bound checkout that task is running in.
- The governing plan path, and the workflow and phase it is being worked under.
- The current `## Next action` from that task file.
