...
- Opposite-posture alternative: `/decide` evidence-grounds each open question per-item before escalating, instead of having Claude answer all of them in one batch. Use `/decide` when explicit per-item evidence + anti-narrowing protection matter more than throughput; use `/recommend` when blanket Claude-judgment-and-go is the right shape.
