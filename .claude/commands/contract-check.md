...
5. **If `$ARGUMENTS` is empty, auto-detect in this order. Stop at the first source that resolves.**
   ...
   b. **Session plan.** Resolve this session's marker via `docs/session-marker.md` § Marker resolution. If `MARKER` resolves, locate the plan file via glob `REPO_ROOT/logs/session-plan-*${MARKER}.md` (matches both date-qualified `session-plan-YYYY-MM-DD-S{N}.md` and bare-marker `session-plan-S{N}.md` forms; prefer the most-recently-modified match). If a match exists AND was last modified today, set `CONTRACT_SOURCE` to that path. The plan's `## Intent`, `## Findings / Items to Address`, and `## Execution Sequence` sections form the contract body. If marker is absent/stale or no file matches, skip this candidate (read-only auxiliary consumer: tolerate marker absence by falling through to candidate (c) — `/contract-check` may be invoked post-session when no active marker exists). Do NOT hard-fail.
