...
5. **If `$ARGUMENTS` is empty, auto-detect.** The current session's mandate is the **last (bottom-most) `## {DATE}` entry block** in `logs/session-notes.md` — the file uses append-to-end ordering, so the final dated block is the most recently started session. Read that block; capture its mandate content as `MANDATE` — the `**Mandate:**` line plus any `In scope` / `Out of scope` / `Stop if` / `Class:` lines that follow it. Those scope clauses are the standard against which drift is judged.
