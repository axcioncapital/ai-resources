...
1. **Re-derive gating independently (non-negotiable).** Before touching anything, decide for yourself whether the item falls in a **gated change class** — structural changes in the high-consequence set (per `ai-resources/docs/audit-discipline.md` § Risk-check change classes), external writes (PR/issue/Slack/upload), destructive git, file deletion outside this session's output scope, or an operator-decision. Use the workspace **Autonomy Rules** + that change-class set as the source of truth. The System Owner's `Gated?` marker is an advisory input only — do **not** rely on it alone (Function A is free-form and may omit it). If your independent check OR the SO marker says gated, treat the item as gated.
