...
- **Design-time gates (not runtime):** landing this command is a new-command structural change — so it took one risk-aware review before landing (`ai-resources/docs/qc-independence.md` § The rule). `/placement` was resolved at design time (index → `ai-resources/logs/archived-projects.md`; archive tier → `<workspace>/archive/`). The command calls neither at runtime.
