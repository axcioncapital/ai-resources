...
19. `/reconcile` is a diagnostic gate. It does not rewrite the target output, does not apply any recommended fix, and does not commit the report. The operator (or a follow-up `/triage` / Codex `$diagnose-and-fix` / `improvement-log.md` entry, per the fix's named channel) decides the next action.
