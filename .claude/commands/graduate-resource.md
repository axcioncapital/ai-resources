...
Skip this step for commands and agents — settings registration is a hook concern. Their distribution is a separate mechanism: the shared-resource auto-sync places them into each consuming project's own `.claude/commands/` and `.claude/agents/` as symlinks to canonical. **`--add-dir` does not do this** — it grants file access and loads `.claude/skills/` only, never `.claude/commands/` or agent types (Claude Code docs, *Skills from additional directories*). If the sync has not run, a graduated command is on disk in canonical and still not invocable from the consuming project.

> **Dispatch timing (session-start registry load).** Once distributed, discovery happens **at session start**, not on write. A command or agent graduated during *this* session is on disk but is **not yet invocable as `/name` or dispatchable by subagent-type name until the next session start.** If a validation or follow-on step in the graduating session depends on dispatching the freshly-graduated agent *by name* (e.g., a workflow acceptance run), it will silently fall back to a `general-purpose` stand-in and leave the real by-name dispatch path unexercised. When that matters, run the validation in a **fresh session** (or restart) after graduation. *(Recorded 2026-06-09 against `/refresh-project-state`'s `project-state-*` agents; generalized here 2026-07-18.)*
