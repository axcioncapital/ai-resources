...
| `logs/improvement-log.md` | T1 (applied-unverified) / T3 (logged/pending/deferred/parked) | Status lines carry bold markup in this log — `**Status:** <value>` — so match the value after the optional `**`, not a bare `Status:`. **T1:** entries whose status is `applied` with no non-empty `Verified:` line. **T3:** entries whose status matches `logged`, `proposed`, `pending`, `logged (pending)`, `deferred`, or `parked` (deferred AND parked both stay active and re-surface per the log's own schema — parking is not archival — so both are still backlog and must be counted). Only entries whose `**Status:**` line contains `resolved`/`RESOLVED` + a `YYYY-MM-DD` date are done — do NOT surface those (a `resolved` token inside proposal prose does not count; only the `**Status:**` line does). |
| `logs/decisions.md` | T2 | Entries carrying a `Resurface trigger` or `Reopen trigger` field. Match the field name allowing an optional parenthetical qualifier and bold markup before the colon — the live form is `**Resurface trigger (concrete, named event):**`, so a hard `Resurface trigger:` literal would miss it. The trigger field is the signal, so a deferred, declined, OR parked decision that carries such a trigger all qualify (do not additionally require the word `Defer` in the title, which misses declined-with-trigger entries). Capture entry date, title, and trigger text |
...
- Improvement-log logged/pending/deferred/parked: <N> items
