...
14. A cleanup plan is destructive, so it takes the risk-aware review row of `ai-resources/docs/qc-independence.md` § The rule — **one** review, before execution. Codex is that review when the session has it. When it does not, spawn one `general-purpose` subagent as the named fallback — **pin `model: opus` on the spawn** — briefed with the seven risk dimensions in addition to the criteria below, and record the result as `unassessed` rather than passed (`qc-independence.md` § When the reviewer cannot be reached). Mirror the path-passing + write-to-disk contract already used by `repo-dd-auditor` and `token-audit-auditor`.
