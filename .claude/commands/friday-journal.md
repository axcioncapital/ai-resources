...
### Step 5.5: Output validation (operator-invoked, not automatic)

15a-pre. **This step does not fire on its own.** After Step 5.4's mechanical check passes, offer it in one line — `Optional: run the journal-scoped review of this report? (y/n)` — and run 15b only on `y`. The report's schema-critical properties are already covered deterministically by Steps 5.4, 5.6 and 5.7, and `/friday-act` disposes every item with the operator before anything executes, so a standing automatic review here is the stacked pass this repo removed on 2026-07-29 (`ai-resources/docs/qc-independence.md` § The rule). On `n`, skip to Step 5.6.

15b. **When the operator asked for it**, spawn one `general-purpose` subagent (fresh context) — **explicitly pin `model: opus` on the spawn**, since `general-purpose` carries no tier of its own. Brief it to run this 6-dimension rubric over the report: **Request Match, Scope Creep, Risky Assumptions, Things That Could Break, Simpler Alternative, Sibling Redundancy.** The focus areas below are scope/context input that steers what it looks for within those dimensions, not a replacement rubric. Pass only the artifact, the scope and the focus areas — never the conversation that produced the report (`ai-resources/docs/qc-independence.md` § Context isolation). Findings are bounded by the materiality floor (`docs/materiality-bar.md`): an observation becomes a Finding only when the reviewer can name a concrete consequence of not fixing it.
  ...
  - **Focus areas (journal-specific — steers evaluation within the standard 6 dimensions):**
    ...
    5. **Risk-class flagging** — items that match `audit-discipline.md` "Structural change classes" (hook edits, permission changes, cross-cutting CLAUDE.md edits, new commands, new symlinks, shared-state automation). Flag these as high-consequence. (A deterministic backstop runs at Step 5.7 — see Notes.)
  ...
  - **Verdict shape:** `GO | REVISE | FLAG FOR EXTERNAL REVIEW`.
...
15e. **On REVISE or FLAG FOR EXTERNAL QC:** run a per-finding disposition loop. For each finding in the summary, prompt:
  ...
  Disposition: (k)eep / (d)rop / (r)evise / (f)lag-high-consequence?
  ```
  ...
  - `r` (revise) — prompt: "Enter replacement directive (must satisfy `^\[(high|med|low)\] .+$`, e.g. `[med] Wire auto-/triage as default after review results`)." Update both the `## Items` line and the matching `### ` heading in `## Item context` to keep the schema contract intact.
  - `f` (flag-high-consequence) — item stays, but its `## Item context` block gains a final bullet: `- **High-consequence:** Yes (matches {class} per audit-discipline.md). This fix takes the risk-aware review row before landing.`
...
15g. Print: `Validation complete. {N} findings dispositioned ({K} kept, {D} dropped, {R} revised, {F} flagged high-consequence). Final item count: {items_generated}.`
...
15p. For each `## Item context` block, scan the directive text + the block's full body for keyword patterns matching `audit-discipline.md` Structural change classes. Patterns (case-insensitive substring match):
...
15q. For each block with at least one match:
  - Check whether the block already has a `- **High-consequence:**` bullet (added by Step 5.5e `f` disposition or a prior run of this step).
  - If absent, append: `- **High-consequence:** Yes (deterministic match: {matched-class}). This fix takes the risk-aware review row before landing.`
...
19. Print to operator:
    ...
    Validation:      {GO | REVISE | FLAG FOR EXTERNAL QC} — {findings count} findings ({K} kept, {D} dropped, {R} revised, {F} flagged high-consequence)
...
- **Output validation (Step 5.5).** Offered — never auto-run — after Step 5 writes the report. Runs as a tier-pinned `general-purpose` dispatch with the 6-dimension rubric inlined (the `qc-reviewer` agent it used to reuse was retired 2026-07-30); journal-specific focus areas (contradictions, currency drift, already-done, vagueness, risk-class, duplicate-merge) are passed as scope context that steers *what* the agent looks for within those dimensions. Per-finding disposition (k/d/r/f) preserves schema integrity: drop = remove both Items line + Item context block; revise = update both halves in lockstep, regex constraint surfaced to operator. Mechanical pre-check (Step 5.4) catches schema breakage before paying for the subagent. The step is offered, not automatic (2026-07-29): the operator answers y/n at 15a-pre, and on `n` it does not run at all. When it does run, the per-finding loop runs to completion. **Vagueness and duplicate-merge focus areas (4 and 6)** ask the agent to attempt resolution from grounding-pass context (Step 4 reads) before flagging — flagging is the fallback when context can't disambiguate, not the default.
...
- **High-consequence flag downstream contract.** Items flagged via Step 5.5e `f` disposition, focus area 5 (subagent), or Step 5.7 (deterministic) all produce the same `**High-consequence:**` bullet shape in their Item context block. Current `/friday-act` behavior: sub-step 15a re-derives the consequence class from the directive text alone, not from the friday-journal Item context block. In most cases the directive text contains the same class keyword that produced the bullet, so the two paths agree — but for items whose Item context flagged a class invisible in the directive text (e.g., directive "Update friday-act required-reads" with Item context referencing `settings.json`), the upstream bullet is silently lost during /friday-act plan-file generation. **Known gap; not fixed in this session.** Follow-up: add to /friday-act sub-step 15a a check that journal-derived items also scan their upstream Item context block for an existing `**High-consequence:**` bullet and honor it. Tracked as a producer-consumer schema gap; the friday-journal side is now the authoritative producer of the flag, but the consumer side does not yet read it.
