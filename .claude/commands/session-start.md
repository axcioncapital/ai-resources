...
3. **`files_in_scope` either lists ≥1 **literal path** OR is explicitly `(inferred)`.** Empty / bare placeholder fails. If `files_inferred = true`, the `(inferred)` marker carries forward to Step 3; this is the only acceptable non-listed shape.

   **Mechanical check — the predicate is "is it a path", not "is it non-empty".** *(Tightened 2026-07-14 after the non-empty test passed a prose footprint five times. Its consumer is a **parser**, not a reader — `check-foreign-staging.sh` tokenises this field on `[,;\s]+` and matches literal paths; prose tokenises into `the`, `18`, `files`, `carried`… , matches nothing, and the guard **fails open**, i.e. the session gets no staging protection at all while appearing to have declared a scope.)*

   For each entry in `files_in_scope`, apply two tests in order:

   a. **Shape test — HARD REJECT.** The entry must look like a path: it contains a `/`, or ends in a known extension (`.md`, `.sh`, `.json`, `.yaml`, `.py`, `.ts`, …), or is a bare `CLAUDE.md`/`SKILL.md`. A glob (`wiki/**/*.md`) passes — the guard handles globs. **Prose fails and is never written.** Re-derive mechanically, or re-ask once: `Files in scope reads as prose, not paths — paste the paths themselves (e.g. from git diff --name-only or find), one per entry.`

   b. **Existence test — HARD REJECT, made safe by routing.** `test -e` each entry. A path that does not exist does not belong in `files_in_scope`.

      **The obvious objection — "but a mandate legitimately names files it will CREATE" — is real, and it is answered by the schema, not by weakening the check.** A file this session will create is not a file in scope; it is a **required output**. The mandate already has a field for it: `- Required outputs:`. Route it there and the existence test becomes a hard reject with **zero false-positive risk**, because everything remaining in `files_in_scope` is by definition something that already exists to be read or edited.

      On a miss, re-ask once: `Files in scope names {N} path(s) that do not exist: {list}. Files this session will CREATE belong in Required outputs, not Files in scope — move them. If any is a typo or a recalled-but-wrong path, correct it.` Apply the answer, then re-test.

      *(A softer "warn and confirm" was the original design here and was cut on 2026-07-14 System-Owner review: a warning is a soft nudge addressed to a model that can rationalise past it, and this check exists precisely because that model's recall is unreliable — five times over. **Do not re-weaken this to a warning.** The clean split between "files I will touch" and "files I will produce" is what makes the hard reject correct, and it also gives `check-foreign-staging.sh` a truer footprint: a not-yet-created output is not a lost-update surface.)*
...
**Model tier and autonomy posture are deliberately absent from this block.** `/session-plan` owns both (its Steps 2 and 5) and discloses them once the plan is written. `/prime` owns `STRUCTURAL_RISK` alone, because it owns the `/risk-check` call that field drives. One field, one owner — the alternative is two derivations that can silently disagree after an `edit`.
