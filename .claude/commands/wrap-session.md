...
3. **Log archive check.** Run `CLAUDE_PROJECT_DIR="$(pwd)" bash logs/scripts/check-archive.sh` — the explicit env prefix is required; some session shells strip the inherited `CLAUDE_PROJECT_DIR` mid-chain, which has caused "CLAUDE_PROJECT_DIR unset" failures across 10+ wraps. The script now archives the **caller's** project (the target it is passed) rather than resolving from its own location, and refuses with `exit 1` if `CLAUDE_PROJECT_DIR` is unset or has no `logs/` — so the prefix is mandatory, not advisory. The script iterates append-only logs (session-notes.md, decisions.md) and archives any file exceeding its threshold by invoking `logs/scripts/split-log.sh` internally. Behavior:
...
5. **Decisions.** If operator decisions with analytical or scoping judgment were made, append at the **END** of `/logs/decisions.md` (newest entry is the LAST entry; do NOT prepend at the top — `check-archive.sh` lists `decisions.md` as `bottom`-ordered exactly like `session-notes.md`, so a top entry is read as oldest and archived) with: date, context, decision, rationale, alternatives considered. **Every appended entry MUST open with its own `## YYYY-MM-DD — {one-line decision title}` header** — that header line is the only thing Step 12d's `--decision-ref-from-header` can address, so an entry written as a bare `**Decision …**` paragraph is *structurally unreferenceable* and produces no orphan to notice: `decisions_refs` simply stays empty, which reads as "a decision-free session". Two decisions in one session get two headers, not one header and a second bold paragraph beneath it. (Undated `**Decision …**` paragraphs *within* one entry remain fine — they are sub-decisions of that entry, and `check-decision-refs.sh` deliberately does not flag them.) Skip if all decisions were routine (operator-directed text edits, QC auto-fixes). If the operator didn't flag decisions but significant ones occurred this session, list them and ask: "Should I log any of these to the decision journal?"
...
6.4. **Session outcome check + value audit — "did Claude do the job well, and was the session worth having?"** Produces an independent verdict on this session along two axes: the mandate verdict (completion + execution quality) and a **Session Value Audit** (80/20 review — session-type worth, opportunity cost, repeat/stop). Advisory only — nothing here blocks the commit or push; a low value SCORE never gates the commit or push. **Commit-verb verification (added 2026-07-03):** before the wrap note is finalized, verify every completed-tense commit claim in it ("committed", "executed", "shipped", "landed") against `git log --oneline` — a commit that has not actually run yet must be written as pending ("commits at wrap"), never asserted done from memory.
   ...
   - **Launch a fresh-context `general-purpose` subagent** with an inline brief (mirror `contract-check.md` Step 4 — NO named agent file, NO disk-notes file; the verdict is short by construction). The brief passes:
     ...
     - **`EXECUTION: OPTIMAL | ACCEPTABLE | SUBOPTIMAL`** — was the *path* good? Judge three sub-axes: **efficiency** (wasted steps, redundant reads, rework loops, unnecessary subagents/detours), **approach** (was a clearly better method/design/tool available?), **process** (required gates followed — or skipped, or over-engineered past the task).
...
12b. **Review status, stated not gated.** If the session touched any structural change class (`ai-resources/docs/audit-discipline.md` § Structural change classes), state in the wrap note whether that change got its independent review, and what the review found. No gate fires here and nothing is commit-blocked: review happens once, at the change, per `ai-resources/docs/qc-independence.md` § The rule — not again at the session boundary. Record `unassessed` where a review could not be reached, so the gap is visible rather than absent. Skip silently if the session touched no class.
