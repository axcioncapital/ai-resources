...
12b. **Review status, stated not gated.** If the session touched any structural change class (`ai-resources/docs/audit-discipline.md` § Risk-check change classes), state in the wrap note whether that change got its independent review, and what the review found. No gate fires here and nothing is commit-blocked: review happens once, at the change, per `ai-resources/docs/qc-independence.md` § The rule — not again at the session boundary. Record `unassessed` where a review could not be reached, so the gap is visible rather than absent. Skip silently if the session touched no class.
