...
3.5. **Foreign-session pre-write guard on `logs/session-notes.md`.**
   ...
   PAIRED CONTRACT — keep in sync.
     ...
     Classifier (id-32, 2026-05-28): when FOREIGN >= 1, the classifier walks `^## YYYY-MM-DD` headers in BOTH WT and HEAD, tracks each mandate's enclosing-date, and emits FOREIGN_CLASS = CONCURRENT (today-extras only) / REMNANT (prior-day-extras only) / MIXED (both) / UNKNOWN (FOREIGN by header-count only — rare; STOPs with a neutral classify-manually message, NOT CONCURRENT-shape remediation). EXTRA_TODAY_MANDATES uses `OWN_MANDATES_SUBTRACT` (marker-aware or PRIME_RAN fallback) as the subtractor.
   -->
...
6.4. **Session outcome check + value audit — "did Claude do the job well, and was the session worth having?"** Produces an independent verdict on this session along two axes: the mandate verdict (completion + execution quality) and a **Session Value Audit** (80/20 review — session-type worth, opportunity cost, repeat/stop). Advisory only — nothing here blocks the commit or push; a low value SCORE never gates the commit or push. **Commit-verb verification (added 2026-07-03):** before the wrap note is finalized, verify every completed-tense commit claim in it ("committed", "executed", "shipped", "landed") against `git log --oneline` — a commit that has not actually run yet must be written as pending ("commits at wrap"), never asserted done from memory.
   ...
   - **Launch a fresh-context `general-purpose` subagent** with an inline brief (mirror `contract-check.md` Step 4 — NO named agent file, NO disk-notes file; the verdict is short by construction). The brief passes:
     ...
     — and the **Session Value Audit** lines (80/20 review — judgmental, skeptical of effort; do not reward effort, length, or complexity; reward only shipped assets, changed decisions, reduced future friction, prevented recurring failures, reusable system improvements):
     ...
     - **Audit evidence rules:** use only what the session actually produced — the session note's Files Created/Modified + Decisions sections and the changed-files/git-log inspection above are the evidence base; no separate evidence list in the output. If there is no concrete evidence, say so plainly; do not inflate. The evidence guard extends to the audit: every D/E classification, FAILED gate, and non-"Correct session" opportunity call must cite a concrete observation.
...
11.5. **Mission update reminder (mission-contract subsystem).** Check whether today's mandate block carried a mission binding: scan from today's `## YYYY-MM-DD` header to the next `##` (or EOF) for a `- Mission: <id>` line. If found, emit one advisory line: "This session served mission `<id>`. Consider updating its `## Open threads` (and closing it if done) with `/mission`." If no `- Mission:` line is present, skip silently. Advisory only — it writes nothing and does not touch the mission file (only `/mission` does). This is a reminder, not the deliberately-killed auto theme-detector — it fires only on an explicit existing binding.
...
12a. **Blind-spot scan nudge.** If the session touched `.claude/commands/`, `skills/`, `.claude/hooks/`, any CLAUDE.md, or made a multi-file change (≥3 files modified), emit one advisory line: "Consider running `/blindspot-scan` before commit — checks stale dependent artifacts, real-usage fit, and capability validity." Do NOT auto-fire the scan; skip silently if no trigger matches. (Paired contract: the workspace-root `wrap-session.md` Step 4.6 carries the same nudge — keep them in sync.)
