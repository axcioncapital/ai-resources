...
- `30_reference-lenses/axcion-stitch-context.md` — the self-contained brand design system for Stitch (the same approved language as the Web Style Lock, with tokens restated so an external tool can hold them). Attach or paste it at the start of the Stitch session; it is what Part 1 below draws from. Advisory and subordinate — outranked by everything above, and by the Web Style Lock it distils.
...
Every Stitch prompt has these six parts, in order. Stitch has no Axcíon context and drifts toward generic SaaS/app UI, so the brand system must reach it one of two ways: attach `30_reference-lenses/axcion-stitch-context.md` at the start of the session (preferred — it is self-contained), or compress its tokens and constraints inline into the parts below. Either way, every token and constraint must be present:

1. **Reference context** — "Design a web page **section** (not an app screen or dashboard) for Axcíon, a Nordic PE/M&A advisory. Use the attached Axcíon {section} as the base. Preserve its dimensions, general grid, approved copy, typography character, {the locked palette}, and restrained use of red; follow the attached Axcíon design system." Attach the standing brand context (`axcion-stitch-context.md`) here. For the *base image*: `baseline.png` is the operator's **current** section render — the starting point to preserve. When you are refining a **known, approved** direction rather than exploring a fresh one, attach the **approved Figma section** screenshot instead and use Stitch's Redesign-from-screenshot path — a visual reference conveys density, whitespace, and rule-weight better than more prose.
...
5. **Visual instructions** — the concrete composition, alignment, type scale, negative space, CTA treatment, and wordmark placement for this direction. Describe visual **mechanics, not adjectives**: do not steer with "premium / professional / minimal / institutional" (Stitch resolves those into generic consulting/SaaS conventions) — state the actual levers (measure, column split, rule weight, spacing step, where the eye lands first).
6. **Prohibitions** — no SaaS cards, gradients, glass effects, dashboards, decorative finance graphics, stock-market imagery, rounded containers, excessive red, or centred landing-page composition — plus any section-specific positioning-hazard tell to avoid. Stitch defaults to SaaS/app patterns (feature-card grids, dashboards, sign-up forms, app chrome/navigation); explicitly reject them. Pair every prohibition with its **positive replacement** — not just "no cards" but "organise the content through alignment, whitespace, hairline rules, and typographic hierarchy"; not just "no rounded corners" but "square geometry, 0px-radius containers." A prohibition without a replacement leaves Stitch to fill the gap with its default.
...
**Iteration (conversational edit).** To refine a good option, do NOT regenerate — instruct an edit on the existing output: "Keep the current composition unchanged. {one precise change}. Do not change the headline, button, colours, or overall composition." **One decision per edit** — combining type, layout, spacing, and content changes in a single instruction makes Stitch reinterpret the whole composition and lose the parts that already worked. Change one thing, look, then change the next.
...
**Font fidelity is not reliable.** Stitch substitutes whatever fonts it has and often misreads weights, so STIX Two Text and IBM Plex Sans will likely render as approximations. Judge the *layout and hierarchy* with the substitute; apply the real typefaces and optical adjustments in Figma. Do not reject a strong layout over its rendered fonts, and do not treat its type as spec.

## Choosing the weight: Lean Exploration → this explore run → the Studio chain

Section work has three tiers of weight — `lean` / `explore` / `chain` — reached by naming the tier, never by the retired word "full". Use the lightest that fits, and escalate only when the **Escalation** checklist below fires. `CLAUDE.md § Section Design Sessions` makes Lean Exploration the default posture and points here for the explore procedure.

**Lean Exploration (the default, pre-approval).** Most early-stage requests — "give me a couple of directions", "some visual exploration", "a Stitch prompt for this section" — do NOT need this 10-step explore run. Run the lean loop instead; you do not have to invoke the whole command to explore:
...
**The explore run (this command) — 10 steps.** Escalate from lean to explore when: choosing between genuinely different compositions; a section is competent but generic and you need to SEE it; the section carries imagery or a custom graphic; lean text directions aren't enough to decide. This tier renders concepts and records `selected-direction.md`.
...
2. **The change departs from a brand rule, positioning hazard, approved copy authority, or an approved section decision** (a **Tier-1 departure**) — route the affected part back through the Studio critics → QC (the chain, i.e. the `visual-design-spec` chain). See the Chain-fit guardrail above.
   - **This is a departure, not staleness.** If an approved section decision is simply out of date because upstream copy or the build brief changed — not because the new design disagrees with the old decision — that's a stale-record problem: reconcile it directly (update the build brief / section record / status file) and do not escalate. Only escalate when the new design itself disagrees with, contradicts, or replaces what was approved.
...
4. **The operator explicitly asks for approval, handoff, or finalization** — move to the chain / handoff.
