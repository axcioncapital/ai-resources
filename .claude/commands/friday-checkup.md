...
14. Check follow-up triggers and assemble three tier-differentiated sections.
    ...
    **Tactical follow-ups (all tiers).** Per-item shape: `[ ] {item} — risk: {low | med | high}`. Risk derivation:
    ...
    - **Session-issue detection:** read `ai-resources/logs/improvement-log.md`; for each entry with a `Category:` line of `session-issue` and a `Status:` line matching `logged (pending)` (same whitespace-tolerant match as Stale-improvement above), add follow-up: `[SESSION-ISSUE] Session issue "{title}" — investigated {date} by /resolve-repo-problem. Decide: fix, defer, or close — risk: med`. Surface regardless of age — these entries are already investigated and carry a proposed fix, so they are fix-ready for `/friday-act`. Date basis: the `### YYYY-MM-DD —` header date.
