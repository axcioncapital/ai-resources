...
14. Check follow-up triggers and assemble three tier-differentiated sections.
    ...
    **Tactical follow-ups (all tiers).** Per-item shape: `[ ] {item} — risk: {low | med | high}`. Risk derivation:
    ...
    - **Fading-gate detection (monthly + quarterly only).** Skip entirely if `TIER=weekly`. For each scope where `is_project=true`, read `{scope_path}/logs/coaching-data.md` if it exists. Parse each session entry (header `### YYYY-MM-DD — {title}`, gate line `**Gates:** N (X changed) — {gate-name}:{outcome}, ...`). Filter to entries dated within the last 30 days from `{TODAY}`. For each `(scope_slug, gate_name)` pair, tally `total` (count of occurrences) and `confirmed` (count where outcome is exactly `confirmed`). For each pair where `total ≥ 8` AND `(confirmed / total) ≥ 0.90`, apply suppression check before flagging: read `ai-resources/logs/gate-calibration.md` (resolved from workspace root, not a per-scope path); operate only on lines outside fenced code blocks and HTML comments; search for a header matching `^## \d{4}-\d{2}-\d{2} — {scope_slug}/{gate_name}$` (U+2014 EM DASH separator, exact match on tokens, case-sensitive, strip leading/trailing whitespace); if multiple entries match, take the first (file is prepend-ordered, so first = most recent); read its `Review-cycle:` line matching `^\s*-\s*\*\*Review-cycle:\*\*\s+(.+)$` (capture is `permanent` literal or ISO date `YYYY-MM-DD`). Suppression outcomes: (a) no matching entry, or `gate-calibration.md` does not exist → flag normally: add follow-up `[FADING-GATE] {scope_slug}/{gate_name}: {confirmed}/{total} confirmed ({pct}%) over last 30 days. Pick: retire / lower-frequency / recalibrate, then record decision in logs/gate-calibration.md — risk: med`; (b) entry found AND `Review-cycle:` is `permanent` or a future date → skip, emit nothing for this pair; (c) entry found AND `Review-cycle:` date has passed → re-flag: `[FADING-GATE] {scope_slug}/{gate_name}: {confirmed}/{total} confirmed ({pct}%) over last 30 days (last calibrated {entry-date} — review-cycle elapsed). Pick: retire / lower-frequency / recalibrate, then record decision in logs/gate-calibration.md — risk: med`. Detection data contract: parse only colon-separated outcome tokens (e.g., `content-review:confirmed`, `plan-approval:changed`); ignore parenthetical context after the outcome token (e.g., `plan-approval:changed (triage execution deferred...)`). Skip silently if `coaching-data.md` does not exist for the scope.
