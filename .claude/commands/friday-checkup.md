...
5. Display the tier plus the auto-run checks that tier will perform:
   ...
   Auto-run checks:
     Weekly:  /audit-repo, /improve, /coach (if ≥5 sessions), /permission-sweep --dry-run, /log-sweep --dry-run; W2.1 doc-scanner + /kb-integrity (if project repo-documentation selected)
...
11. Compute rough runtime estimate using these per-check ceilings:
    ...
    - `/log-sweep --dry-run` = 2 min (ai-resources + active projects)
    ...
    - W2.1 doc-scanner (Step H) = 3 min (if `project repo-documentation` selected — all tiers after D-39)
    - `/kb-integrity` (Step J) = 2 min (if `project repo-documentation` selected — all tiers after D-39)
...
**G. `/log-sweep --dry-run` — all tiers, once per checkup run**

Log-sweep scans `ai-resources/` and all selected project scopes in one pass — it is **not** per-scope in the sense of requiring separate invocations. Invoke it exactly once regardless of how many scopes are selected.

1. `cd "{WORKSPACE_ROOT}"` (the workspace root, not a scope path).
2. Verify `ai-resources/.claude/agents/log-sweep-auditor.md` exists. If missing, record `skipped: log-sweep-auditor not deployed` and continue.
3. Verify `ai-resources/logs/scripts/log-archiver.sh` exists. If missing, record `skipped: log-archiver.sh not deployed` and continue.
4. Invoke `/log-sweep --dry-run`. The command runs the auditor across all discovered scopes, writes per-scope working notes, and produces a dry-run report — no archive operations are applied.
5. The command writes its dry-run report to `ai-resources/audits/log-sweep-{TODAY}.md`.
6. Record the report path in `RESULTS` under a synthetic scope label `log-sweep (workspace-wide)`.

**H. W2.1 — `doc-scanner-agent` (repo-documentation only) — all tiers**
...
**I. W2.2 — `principles-checker-agent` (repo-documentation only) — monthly and quarterly only**
...
**J. W2.3 — Maintenance consolidator (repo-documentation only) — all tiers**

Skip if scope `project repo-documentation` is not selected. Runs AFTER §H (W2.1) so the drift report exists; runs after §K (W2.4 improvement-analyst) is irrelevant — independent.

This is an orchestration step (no separate agent — D-7). It consolidates the §H drift report and `/kb-integrity` output into a single maintenance summary.

1. Read the §H W2.1 drift report at `projects/repo-documentation/output/phase-2/w2-1-doc-scan-{TODAY}.md`. If missing (§H skipped or failed), record `skipped: w2-1 prerequisite missing` and continue.
...
3. Compose a consolidated maintenance summary at `projects/repo-documentation/output/phase-2/w2-3-maintenance-{TODAY}.md` with sections:
   - **Drift since last scan** — copied from §H drift report (Added/Removed/Modified counts and one-line per finding).
   ...
   - **Recommended actions** — operator-facing list combining both: registry pastes (from §H), drift investigations, integrity fixes.
4. Record the consolidated report path in `RESULTS` under scope label `repo-documentation:w2-3-maintenance`.

**K. W2.4 — `improvement-analyst` (Phase 2 cadence-driven catch-up) — monthly and quarterly only**
...
**L. Projects-layer refresh (repo-documentation only) — monthly and quarterly only**

Skip entirely if `TIER=weekly`. Skip if scope `project repo-documentation` is not selected.

*After D-38 (2026-05-06), `output/phase-1/components/` is a frozen archive. L updates vault narrative-note paths only (`vault/projects/`, `vault/architecture/`). The component-registry version of `projects.md` (at `vault/components/projects.md`) is updated via `/kb-update` or W2.1, not here.*
...
14. Check follow-up triggers and assemble three tier-differentiated sections.
    ...
    **Tactical follow-ups (all tiers).** Per-item shape: `[ ] {item} — risk: {low | med | high}`. Risk derivation:
    ...
    - **Stale-improvement detection:** read `ai-resources/logs/improvement-log.md`; for each entry whose `Status:` line matches `logged (pending)` — the de facto convention per D-33/D-40 (do NOT match `Status: pending` alone; no current entries use that form) — **and whose `Category:` is not `session-issue`** (session-issue entries are surfaced by Session-issue detection below, regardless of age — do not double-list them here) — compute days elapsed from the entry's most recent date stamp (the `### YYYY-MM-DD —` header date, or the most recent `Review-cycle:` date if present — the latter is an explicit deferral signal and resets the clock). If days elapsed > 21, add follow-up: `[STALE] Improvement entry "{title}" pending {N} days. Decide: apply, defer with explicit Review-cycle note, or close — risk: med`. Data contract: match literal `Status:` followed by `logged (pending)` (whitespace-tolerant; allows bold markers like `**Status:** logged (pending)`), and skip any entry whose `Category:` line is `session-issue`; age basis is `Review-cycle:` date if present, else `### YYYY-MM-DD —` header date.
    - **Session-issue detection (legacy drain):** read `ai-resources/logs/improvement-log.md`; for each entry with a `Category:` line of `session-issue` and a `Status:` line matching `logged (pending)` (same whitespace-tolerant match as Stale-improvement above), add follow-up: `[SESSION-ISSUE] Legacy session issue "{title}" — investigated {date}. Decide: fix, defer, or close — risk: med`. Surface regardless of age — these entries were written by the retired `/resolve-repo-problem` path and already carry a proposed fix, so they remain fix-ready for `/friday-act`. New specific repository faults go to Codex `$resolve-repository-problem` and do not enter this legacy queue automatically. Date basis: the `### YYYY-MM-DD —` header date.
    - **Open-items backlog surfacing (all tiers; operator directive 2026-07-03).** Run the `/open-items` scan logic (sources, tiers, hard exclusions, and reconcile-at-read per `.claude/commands/open-items.md`) for `ai-resources` plus each selected project scope that has a `logs/` directory. Take the PRIORITY section plus Tier 1 (unresolved friction, active inbox briefs, next-up checkboxes, applied-unverified improvements, recent session-plan steps). Emit each as a follow-up: `[OPEN-ITEM] {item} — {source path} — risk: med` (PRIORITY-promoted items → `risk: high`). Dedupe first against items already emitted by the Stale-improvement / Session-issue / Park-drain items above (match by title). Cap: 10 `[OPEN-ITEM]` lines across all scopes, oldest first; if more qualify, add one final line `[OPEN-ITEM] +{N} more — run /open-items in {scope} for the full list — risk: low`. Purpose: open backlog items flow into `/friday-act` on the cadence instead of waiting for an operator-invoked `/open-items` run.
    - **Defect-log recurrence scan (all tiers).** Read `ai-resources/logs/defect-log.md`. Scan the entries below the `<!-- entries below, most recent first -->` marker. Group entries by `**Class:**` value and, per class, count entries whose `**Action:**` line is exactly `captured` (routed entries — `routed → …` — are excluded; they are already closure work). For each class with a captured-count of **2 or more**, the class has recurred while still un-routed — this is the signal the loop exists to surface. Add one follow-up per such class: `[DEFECT-RECURRENCE] Defect class \`{class}\` has {N} un-routed (captured) entries — route it to a rule / eval / example per docs/defect-to-fix-loop.md § The three routes, then mark the entries routed — risk: med`. This is a trivial single-file grep — run it inline, no subagent. If `defect-log.md` is absent or no class has ≥2 captured entries, emit nothing for this item. (Cadence note: this fires on every tier — weekly coverage satisfies the loop's "fortnightly-or-better" recurrence-scan intent; the scan is cheap enough that gating it to monthly+ would only let recurrences sit unflagged. Routing itself stays gated judgment work for `/friday-act`.)
    - **Fading-gate detection (monthly + quarterly only).** Skip entirely if `TIER=weekly`. Spawn the `fading-gate-scanner` subagent with: `TODAY`, the list of `(scope_slug, scope_path)` pairs from Step 4 where `is_project=true`, the absolute path to `ai-resources/logs/gate-calibration.md`, and `AI_RESOURCES/audits/working/` as the working dir. The subagent reads each `{scope_path}/logs/coaching-data.md` (up to 9 files), tallies per `(scope_slug, gate_name)` over the last 30 days, applies the suppression check against `gate-calibration.md`, and returns a `[FADING-GATE]` follow-up line per surviving candidate (≤30-line cap; full list to `audits/working/fading-gate-scan-{DATE}.md` if findings count >20). Append the returned lines verbatim to the Tactical follow-ups list. If the subagent returns `(no fading gates)`, emit nothing for this item. **Full data contract** (em-dash separator, parenthetical handling, header regex, suppression semantics, output shape) lives in the subagent body at `ai-resources/.claude/agents/fading-gate-scanner.md` — do not duplicate it inline here.
    - **Park-drain (monthly + quarterly only).** Skip entirely if `TIER=weekly`. This is the forcing review that keeps the `Review-cycle:` park from becoming a graveyard (per workspace `CLAUDE.md` § Working Principles + `improvement-log.md` schema): a low-value item parked via `Review-cycle:` resets its stale-scan clock, so an item that is re-parked every cycle could hide indefinitely. Read `ai-resources/logs/improvement-log.md`; collect every entry that carries a `**Review-cycle:**` deferral line (the canonical park signal). For each, compute age from the **original `### YYYY-MM-DD —` header date** — NOT the `Review-cycle:` date — so repeatedly-re-parked items surface by true age. Sort oldest-first and surface up to the **5 oldest** as: `[PARKED] Improvement entry "{title}" parked since {header date} ({N} days), last re-parked {Review-cycle date}. Decide: do it now (structural fix), re-park with a fresh concrete trigger, or close — risk: med`. Data contract: match `**Review-cycle:**` (whitespace/bold-tolerant); age basis is the `### YYYY-MM-DD —` header date (the deliberate inverse of the Stale-improvement rule, which uses the Review-cycle date). If no entry carries a `Review-cycle:` line, emit nothing for this item.
    - **Friction-log dormancy:** read `ai-resources/logs/friction-log.md`. Extract the most recent entry date (the most recent `### YYYY-MM-DD` or `## YYYY-MM-DD` header). Compute days since that date. If file exists and days elapsed > 21, add follow-up: `[FRICTION-DORMANT] Friction-log last entry: {date} ({N} days ago). Either no friction occurred (good), or friction is going unlogged — review whether friction-log still earns its keep — risk: low`. If friction-log does not exist, skip silently.
    - **Archaeology-baseline staleness (repo-documentation):** read line 2 of `projects/repo-documentation/output/phase-1/inventory/components.md` and extract the date from its `_Generated: YYYY-MM-DD` line; if the line is absent or unparseable, fall back to the file's mtime (`stat -f %m` / `stat -c %Y`). Compute days elapsed. If > 28, add follow-up: `[ARCHAEOLOGY-STALE] Inventory baseline last generated {date} ({N} days ago; threshold 28). Run /archaeology in projects/repo-documentation to refresh the component baseline — risk: low`. Advisory only — no formal re-run cadence is mandated; this nudge is the cadence mechanism (the vault/SO grounding updates via W2.1, not from the inventory, so a stale baseline degrades scope tracking, not advice — see `projects/repo-documentation/references/phase-2-cadence.md`). If the inventory file is absent (project not present on this machine), skip silently. Existence-gated, all tiers, regardless of scope selection — same precedent as the §M anchor check: the check is one file read, too cheap to gate.
    ...
    - **Unpushed commits:** run `git log @{u}..HEAD --oneline 2>/dev/null` in both `ai-resources/` and the workspace root. For each scope with non-empty output, count commits and add `` `git push` — {scope}: {N} unpushed commit(s) `` (risk: `med`). Empty output (no unpushed commits, or no upstream tracked) → no follow-up.
