...
For each fix you recommend, name its level (output/workflow/repo) and its closure channel per `reconcile-failure-taxonomy.md` § Fix levels — `/triage`, Codex `$diagnose-and-fix` or a direct project edit, or `logs/improvement-log.md` or a direct rubric/map edit. A fix with no named channel is an incomplete finding — either name the channel or do not list the fix.
