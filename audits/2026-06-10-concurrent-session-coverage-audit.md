...
## Post-landing notes (2026-06-11 S2 — P1–P4 committed after independent QC GO)

- **P1/P2 LANDED** (ai-resources dd44220 + positioning-research 15220f4): both hooks now register ONCE at user level (`~/.claude/settings.json`, machine-absolute paths); the three repo/project-level registrations are deleted (atomic landing). **P3 LANDED** (same commit): the no-footprint fail-open now splits BLOCK (live foreign per-id marker) vs warn+allow. **P4 LANDED** (workspace-root cafdb57): wrap Step 13 marker teardown ported. P5 stays deferred.
- **R1 — Daniel handoff (load-bearing):** the deleted repo-level registrations used `$CLAUDE_PROJECT_DIR` (portable — they covered Daniel's clone). The user-level replacement is machine-absolute, so **Daniel now has ZERO concurrent-session coverage until he adds both hooks to his own `~/.claude/settings.json` with his clone's absolute paths.** Patrik: relay this to Daniel. Hook paths to register: `<his-clone>/ai-resources/.claude/hooks/check-foreign-staging.sh` (PreToolUse → Bash, timeout 5) and `<his-clone>/ai-resources/.claude/hooks/detect-concurrent-session.sh` (SessionStart, timeout 5).
- **R2 — orphan hook copy:** `projects/research-pe-regime-shift-advisory-gap/.claude/hooks/detect-concurrent-session.sh` exists but is registered nowhere. Status: deliberately-unregistered as of 2026-06-11 — it must NOT be registered (would re-introduce double-fire). Cheap cleanup for a session in that repo: delete the file.
- **First-firing defects (logged 2026-06-11 improvement-log):** the P3 guard's first live firing exposed (a) the mandate-header lookup matches `Session S{N}` without anchoring the DATE, so a same-numbered older session's footprint can shadow today's; (b) a `/handoff`-ended session leaves its per-id marker un-torn (wrap Step 13 never runs), which P3 reads as a live foreign session → hard BLOCK. Both pending a gated fix.
