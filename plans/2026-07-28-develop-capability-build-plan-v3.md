# Build `/develop-capability` — the execution-and-proof loop for approved Axcíon capabilities (v3)
...
>
> **Retained as review trail and drafting source only.** Its §§7–10, 12, 14, 16 and Appendix A supplied text for the shipped skill. Where this file and the consolidated plan disagree, the consolidated plan wins without exception.
>
> Banner added 2026-07-28 (S6-ceb), remediating a finding from `audits/risk-checks/2026-07-28-retrospective-work-loop-session-a-b-landed-set.md`: this file was left dangling with a status line claiming it awaited approval, which was no longer true.

**Status:** ~~DRAFT v3 — awaiting operator approval~~ → **SUPERSEDED 2026-07-28.** Never approved, never implemented; see the banner above. No repository change was ever made from this document directly.
**Supersedes:** v2 (premise void) and v1 (superseded). Both retained as the review trail.
...
- **independent review of the built result** — all four Codex reviews sit at Phases 6, 8, 9 and 12, every one of them *before* implementation (`workflow.md:1002-1024`);
...
> **Systems Builder decides what to build and why. `/new-project` sets the project up to build it. `/develop-capability` builds it, proves it, puts it into real use, and closes it.**

### Operator constraints binding this version

1. Systems Builder is the **first host and pilot environment**, not a prerequisite. Nothing here waits on a Systems Builder redesign.
...
4. **Systems Builder is not modified or redesigned** by this implementation. No file under `projects/axcion-systems-builder/` is touched.
...
Four things distinguish v3 from its predecessors.
...
**It changes nothing it does not own.** Systems Builder is untouched. Where the two overlap, § 26 records the observation and leaves it for the post-pilot review.
...
| **F34** | **All four Codex reviews are pre-implementation** — Needs quality, V2 product definition, MVP discipline, V3 technical solution. None reviews a built result or a real use. | `workflow.md:1002-1024` |
...
| **F36** | Core Operating Rule 4: *"Manual work is acceptable during the MVP. A controlled manual process is preferable to premature infrastructure when the operating model has not yet been proven."* | `workflow.md:1046-1048` |
...
| **F40** | **`contacting-operations` is at Phase 4** — V1 received 2026-07-24, 8-document set, 18 open questions. `02`–`06` are stubs. The register describes `email-system` as *"the technical email machine that implements `contacting-operations`"* — the seam between them is already stated. | `cases/README.md:35` |
...
| **F43** | `workflow.md` closes by anticipating future tooling: *"This can be converted next into a lean Systems Builder command specification without adding unnecessary governance."* | `workflow.md:1066` |
...
| **D14** | **No file under `projects/axcion-systems-builder/` is created, edited or deleted by this implementation.** | Operator constraint 4. Also protects the pilot: a host modified by the thing it hosts cannot evidence anything. | Adding a Phase 15 pointer to `workflow.md` — deferred to § 26 O-1. |
...
`/develop-capability [outcome or observed need]` · `/develop-capability adopt-from-case {path}` · bare, to resume.
...
**Prong (b), evidenced failure — PASSES.** `docs/ai-resource-creation.md:27` requires *"a failure mode with cited written evidence… or a pattern seen ≥2 times."* Both limbs are met:
...
2. **The structural gap is documented in the workflow's own text.** Phase 15 is one paragraph (F35), and all four independent reviews sit before implementation (F34). The process that governs Axcíon's system development defines its front half in ~900 lines and its execution half in fourteen.
...
**Honesty note for the reviewer.** Prong (b) is met by evidence about *systems built under a previous process*, not by an observed failure of this specific gap. A reviewer who judges that too indirect should say so; the fallback is the OP-11 route, and § 27 OQ-1 puts that question to the operator rather than assuming the answer.
...
**Exactly one Heavy stop exists, and it is G1 at the end of Shape.**
...
4. **Walk the intervention ladder** and name the rung: accept the limitation → change an operating habit → clarify ownership or information flow → reuse an existing capability → simplify or remove the source → narrow local improvement → bounded experiment → build. Say why not the rung below.
...
24. **Operator stop G3 — the lifecycle decision.** One of eight terminal statuses. `paused` requires a reopening trigger.
...
14. **Conflicts are surfaced, not silently resolved.** 15. **A control that cannot run reports "unassessed," never "passed."** 16. **Nothing under `projects/axcion-systems-builder/` is modified** (D14).
...
Failure conditions, reported as such: an adverse review verdict · a blocking unknown no evidence can close (produce a requirements doc and stop) · a material scope change altering seam, owner or exclusions · two authoritative sources in genuine conflict · escalation into Heavy during Build or later · a confidentiality question the rules do not answer · a Systems Builder register item the capability depends on that is still open (e.g. `email-system` A2/A3, F39) · any of the ten workspace pause triggers.
...
| Carrying a sibling change request (§ 14.9) | H3 "a second project must change" | Yes, for slices that do not depend on it |
...
Discovery is a glob over `development/*.md` frontmatter filtered by the active set. No index, no register. More than roughly a dozen records in one project signals the project should split.
...
**`ai-resources/.claude/commands/develop-capability.md`** — 150–220 lines, `model: opus`. Owns invocation and arguments · project and state discovery · **`adopt-from-case` intake, including rehearsal mode** (read-only when the case has no owning project yet — reports what it would consume, where it would enter and what would pause it; opens no record and writes no file; § 25) · resume detection · lane routing and announcement · phase orchestration · operator gates and interactions · handoff execution and resumption · record read/write · `WORKDIR` lifecycle · reporting · failure behaviour.
...
- **Must not.** Create the project. Write into `projects/project-planning/`. **Write into `projects/axcion-systems-builder/`** — the operator opens the case there (F38: case material is captured by hand). Leave the capability "open pending a project."
...
- **Back.** One of the **eight terminal-or-active statuses selectable at Land** — `adopted`, `continue-trial`, `revise`, `keep-local`, `paused`, `closed`, `retired`, `rejected`. (`in-development` is not selectable here.)
...
- **Back.** The named evidence, carried by the operator.
- **Next.** Record it under `## Change requests` (disposition + returned evidence) and `## Verification evidence`. Resume the dependent slice.
- **Receiving mechanism, named (QC finding 2).** **The operator carries and executes it**, in a session opened in the sibling project. `/develop-capability` never opens that session and never writes there. This is counted as an **interaction** in § 11.6.
...
- **Out.** Nothing. This is an **intake**, and it is **read-only** (D14).
...
- **Partial cases — the normal state today.** `email-system` has `01` and `02`; `03`–`06` are stubs (F39). `contacting-operations` has `01` only (F40). The command **must not** treat a stub as an approved artifact. Rule: read each artifact's `**Status:**` line; anything reading *not started* is absent. With `02` approved but `04` absent, the capability may enter at **Shape** using `02` as the verified need and author its own slices — recording explicitly that the MVP boundary was not available and that any slice it authors is provisional against a future `04`.
- **Next.** Record everything consumed, its state at consumption, and **what was not re-derived**, under `## Adopted case material`. Set `source_case:` in frontmatter.
- **Must not.** Create, edit or delete any file under `projects/axcion-systems-builder/` (D14). Re-run Phases 4–13. Reopen an operator-approved definition. Treat a stub as content. Resolve a still-open register item by inference — `email-system` A2 (who builds and operates it) and A3 (the compliance source) are open (F39), and an unresolved dependency on either is an exceptional pause (§ 11.3), not an assumption.
...
1. Glob `projects/{cwd-project}/development/*.md`; frontmatter only.
...
| **A case register item the capability depends on is open** | Exceptional pause (§ 11.3). Name the item. Do not infer an answer. |
...
**Lane** Heavy. **Why** H1 — the buyer record sits in an external CRM which the Needs Document establishes as authoritative and outside the Email OS, and Gmail is the official send record. H2 — a real buyer record is real relationship data. **H4 does not fire on the draft**: producing a draft for human review is reversible; sending is not, and sending is outside this capability. H5 likely — the language layer is claimed by `axcion-communication-system` (F28). **Owning project — named, because D14 makes it non-obvious.** The record does **not** live in `axcion-systems-builder`; that repo holds the *case*, is read-only to this command, and holds no operating capability. The owner is **the project `/new-project` creates from the `email-system` package at workflow Phase 14**, which does not exist yet (F39). Its name is set when that project is created; until then this capability has no home for a record, which is exactly the constraint § 25 works around. `buy-side-service-plan` is **not** the owner either — it holds needs analysis and a coverage gap report (F41), not the operating outcome. **Artifacts** `projects/{phase-14-project}/development/buyer-email-preparation.md` with `source_case: projects/axcion-systems-builder/cases/email-system/`; `## Adopted case material` recording what was consumed; two Codex briefs in `WORKDIR`; a § 14.3 brief for any drafting artifact. **Review depth** Codex ×2 targeting the built result and release (the definition was reviewed at Phases 6–12); `/risk-check` if a skill or command is authored; `/qc-pass` on that artifact. **Handoffs** § 14.10 *adopt-from-case*; § 14.3; § 14.6; § 14.7; § 14.9 if the CRM's owning project must change. **Evidence** one real draft from one real record, produced in `WORKDIR`, read by the operator, compared against the guidance; never committed. **Decision** *continue trial* — one draft is not adoption. Stops **3**.
...
**Lane** Heavy. **Why** H5 — it defines decisions currently unowned, adjacent to territory `axcion-communication-system` holds, and will claim part of that authority surface. **H3 does not fire on EmailOS as a consumer**: `email-system` exists as a case but not as a built capability, and § 6.2's H3 test excludes a consumer that does not yet exist — counting it would be the speculative-abstraction pattern. **Artifacts** the record with `source_case: .../cases/contacting-operations/`; `## Adopted case material` noting that only `01` exists (F40), so the capability enters at Frame, not Build; the strategy documents themselves in the owning project. **Review depth** Codex on the package; brief #2 light if nothing executable was built — **say so explicitly rather than skipping it silently**; `/risk-check` only if a class fires; `/qc-pass` on the strategy document. **Handoffs** § 14.10 (partial case); § 14.6; § 14.7. **Evidence** the strategy applied to one real contact decision, and that decision compared against what the operator would have done unaided. **Decision** *adopt* if the real decision holds up; *keep local* if it serves only one project.
...
| C4 | `ai-resources/plans/2026-07-28-develop-capability-build-plan-v3.md` | This plan | — |
...
**Not edited:** `prime.md` (CX-1). **Nothing under `projects/axcion-systems-builder/`** (D14) — no file created, edited or deleted there by this implementation.
...
four Codex red teams, all of them before implementation (`workflow.md:1002-1024`). Phase 15,
...
**Systems Builder is not modified by this change.** No file under
`projects/axcion-systems-builder/` is created, edited or deleted. Overlaps between the new
command and existing phases are recorded as follow-up observations in build-plan § 26 and
left for the post-pilot review, per operator direction 2026-07-28.
...
Plan: `plans/2026-07-28-develop-capability-build-plan-v3.md`.
```
...
| **`projects/axcion-systems-builder/`** | **Unchanged — read-only consumer relationship** (D14). Its cases become input to a command that writes nothing there | **None by construction**; verified by AT-27 |
...
| AT-24 | **Change request is durable and enabled** (QC finding 2) | trigger H3 "a second project must change" | request appears **verbatim in `## Change requests`**, survives `WORKDIR` disposal, names the operator as executor, and records the returned evidence; **zero writes** to the sibling tree |
...
| **AT-27** | **Systems Builder untouched** (D14) | after every commit: `git -C projects/axcion-systems-builder status --porcelain` | **empty output**. Also `git diff` over the whole change set shows no path under `projects/axcion-systems-builder/` |
...
| **AT-29b** | **Rehearsal mode when no owning project exists** (§ 25) | run `adopt-from-case` against `cases/email-system/` with no Phase-14 project created | the command produces a chat report naming what it would consume, what it treats as absent, where it would enter, and which open register items would pause it — and **writes no file and opens no record**. Verified by `git status --porcelain` being empty across every repo afterwards |
| **AT-30** | **An open register item pauses** | run against a case with an open item the capability depends on (`email-system` A2) | exceptional pause; the item named; no inferred answer |
...
| R11 | **Record becomes a second source of truth** against project authority documents or case artifacts | Medium | Medium | The record points, never restates. Case artifacts are read-only (D14). `## Adopted case material` records what was consumed rather than copying it | Records are advisory markdown |
| R12 | **Scope creep into project management or into Systems Builder's remit** | Medium | Medium | § 4.3's exclusions name Phases 4–13 explicitly as excluded; D14 and AT-27 make the file boundary mechanical | Narrow the trigger in the command file |
...
| R14 | **The pilot cannot run when planned** because `email-system` is at Phase 6 with stub artifacts | **High** | Medium | § 25 states the dependency honestly; § 14.10's partial-case path lets the command run against `02` alone; CX-4's Lightweight-first pilot is unaffected | Pilot the Lightweight case; hold the EmailOS pilot until Phase 14 |
...
- **`projects/axcion-systems-builder`** — **nothing to roll back.** No commit touches it (D14, AT-27).
...
| **Any change to Systems Builder** — including naming `/develop-capability` in `workflow.md` Phase 15 | Operator constraint 4 | The post-pilot review (§ 26 O-1) |
...
| Immediately after implementation | **`adopt-from-case` intake against `cases/email-system/` in *rehearsal mode*** — see below. Confirms the command reads `02`, treats `03`–`06` as absent, identifies where it would enter, and pauses on A2/A3. AT-28, AT-29 and AT-30 run against real material | Nothing — the case exists today |
| When `email-system` reaches Phase 13/14 | **The full Heavy pilot** — slices from `04`, seam from `05`, acceptance from `06` | Systems Builder Phases 6–13 completing, **and the Phase-14 project existing** |

**The owning project does not exist yet, and that shapes the pilot.** The EmailOS capability's owner is the project `/new-project` creates from the `email-system` package at Phase 14 (§ 17.2). Systems Builder holds the case but is read-only to this command (D14) and is not the owner. So there is nowhere to write a capability record until that project exists.

**Rehearsal mode — the small design consequence, stated rather than papered over.** When `adopt-from-case` is invoked and **no owning project exists yet**, the command runs read-only: it reads the case, reports which artifacts it would consume, which it treats as absent by their `**Status:**` line, where it would enter (Frame / Shape / Build), and which open register items would pause it — and it **opens no record and writes no file**. The output is a chat report. This is not a lane and not a lifecycle; it is an intake dry run, and it is the only form the EmailOS pilot can take before Phase 14. It exercises everything in `adopt-from-case` except the record write, which is the part already covered by every Medium and Heavy case. Rehearsal mode is named in the command's `adopt-from-case` intake (§ 13.1) and tested by AT-29b.
...
|---|---|---|
| **O-1** | **`workflow.md` Phase 15 does not name `/develop-capability`.** Naming it would fully discharge RR-05's letter (§ 4.5) and make the pipeline position explicit rather than interpreted. | Modifying Systems Builder is forbidden here (D14). Deferring it also means the pilot tests the command before the workflow depends on it. |
...
| **O-6** | **`workflow.md:1066` anticipates "a lean Systems Builder command specification."** If that is built, its boundary with `/develop-capability` must be settled deliberately rather than by whichever ships first. | Not this implementation's call. |
...
|---|---|---|---|
| **OQ-1** | **Is prong (b) accepted as clearing the complexity budget (§ 4.4), or do you want an OP-11 exception written anyway?** | **Prong (b) clears it; no OP-11.** | The evidence is about systems built under a previous process, not a direct failure of this gap. If you or `/risk-check` judge that too indirect, the fallback is OP-11 and § 18.5's entry changes shape. |
| **OQ-2** | **If `/placement` recommends a location or name other than `projects/{p}/development/`, I apply it and report.** Confirm only if you want the opposite. | Apply and report. | Placement is a technical call; nothing is built against the name yet. |
| **OQ-3** | **Accepted resumption gap (R1, U6)** — no automatic surfacing of an abandoned capability. Comfortable relying on `/wrap-session` → `/prime` Step 1b plus naming it yourself? | **Yes**, per **CX-1** (§ 2.4) — the Codex answer that deferred the scanner. | If you expect to abandon Heavy capabilities without wrapping, § 23's trigger fires early. |
| **OQ-4** | **Pilot sequencing (§ 25).** The full EmailOS Heavy pilot needs `email-system` at Phase 13/14; it is at Phase 6. Run the Lightweight pilot plus an `adopt-from-case` intake test now, and the Heavy pilot when the case advances? | **Yes** — that is what § 25 plans. | It is the one place your instruction and repository state do not currently line up. Worth an explicit yes. |
...
| 4 Smallest sufficient and proportionate | Three lanes; zero files at Lightweight; § 11's stop and interaction tables; § 23's exclusions; the deferred `/prime` scanner; D14's no-touch rule |
...
| 7 Completion includes delivery, use and closure | Land in every lane; adoption requires observed use **and** operator acceptance; eight terminal statuses; **this principle is the gap the command fills** (§ 0B) |
...
**End of plan v3.** No repository change has been made. Systems Builder is untouched.
