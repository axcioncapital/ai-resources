...
- **Dependencies:** G1 cleared (on top of the § 5 authority baseline); S3 useful but not required. **The execution path turns on whether the runtime has landed, not on how G2 resolved — which is what keeps G1 and G2 independently decidable and S5 independent of S4:** R1/R2 execute retrieval through the runtime **once S4 has landed**, and on the **manual execution model** in every state where it has not — G2 rejected, and equally G2 approved with S4 still pending. On that manual path the assigned tool still executes the research, driven from one batched prompt block per route run with results pasted back (the lean proposal's own fallback design), and the route's acceptance evidence additionally records the operator relay burden per run. S4, whenever it lands, upgrades the execution path without changing the route contract, so S5 never waits for S4. **Gate:** G1.
...
**Sequence:** the G-session (§ 7) comes **first** — its approval of this plan (G4) is the baseline authority every slice needs, and no slice starts before it (or a separate per-slice authorization). Then S1 → S2 → S3 (substrate, in order of increasing blast radius; S3 also requires its G5 confirmation from that session); S6 as soon as a real unit exists (independent track); S4 as G2 allows; S5 after G1 — through the runtime once S4 has landed, and on the manual execution model in every state where it has not. Earliest real operating proof arrives from S6 and the first real route runs — all before broad rollout (S5 adoption beyond the first real uses, deep-pipeline slimming, consuming-project propagation are all behind that evidence).
