...
| # | Path | Change | Risk |
|---|---|---|---|
| **E1** | `develop-ai-resource.md` | Boundary bullet + Step 1.0 upstream-brief clause, retargeted to `/work-loop`, **plus the 2026-07-28 provenance-verification tightening** (four checks, malformed-handoff path, Step-4 upstream branch). ~10 lines as first shipped; **~40 after the tightening** | Low — additive, and fails closed |
...
| **5** | B | `update: /work-loop — capability units, record correlation, cardinality` | C3 | A-CAP-1..7, **AT-7**, A-RES-1..2, A-REC-2 | One solo capability unit and one reviewed capability across ≥2 correlated units; all four ACTIVE statuses resumable |
| **6** | B | `update: handoff contracts` | E1, E3, E5 | A-HAND-1..5 | Verified upstream brief not re-qualified; malformed handoff reported and fully qualified; plain invocation unchanged and silent |
...
- **A-CORE-3 — Sizes.** `wc -l`: C1 ≤**260**; C3 at or below its recorded ceiling (§13).

  **Ceiling amended 220 → 260 at commit 3, 2026-07-28.** Second amendment; the first (180 → 220) is recorded below and its reasoning applies unchanged. The increase covers exactly two additions, both of which are **durable reference material that §13's permitted-response (2) explicitly directs into C1** rather than into the command:

  (a) **§ The challenged route — gates and review placement** (~20 lines). The route's three stops were named in the route→depth→stops table and defined nowhere. G1/G2/G3 — what each gate sits at, what the operator is deciding, and what package is held — is reference the Codex controller and `/work-loop` both read, and it is exactly the "phase table" class the response rule names. It also states *why* the two reviews are distinct files (one unit per phase ⇒ different units ⇒ different files by construction), which is the property A-CHAL-2 tests.

  (b) **Correlation's cross-session requirement** (~4 lines). A Prove unit must read its stream's Shape review **on disk** days later; that works only because the stream id keeps its original date *and* retention is per stream. Both rules already existed separately; what was missing was the statement that neither is sufficient alone — and the prohibition on re-deriving a missing phase artifact from memory, which would silently collapse the two reviews into one.

  **The alternative was putting both in C3, and it was rejected on the boundary, not on the number.** C3 is at 212 of 300 and had room. But G1/G2/G3 definitions in the command would leave the Codex controller reading gate semantics out of a Claude-side orchestrator, and A-CORE-7 — which outranks A-CORE-3 — tests that C3 holds orchestration and not durable reference. A number that was an estimate does not outrank a boundary that was designed.
...
- **AT-7 — Resume sees the whole ACTIVE status set, not just `in-development`.** Construct four fixture records, one at each ACTIVE status — `in-development`, `continue-trial`, `revise`, `paused` (the last with a `reopen_trigger:`, since one without is malformed). Invoke `/work-loop` bare. **Pass:** all four are listed as resume candidates. **Fail:** any tier returns only the `in-development` record.

  **Why this test exists, and why it is not optional.** Commit 4 shipped C5's ability to *write* all four ACTIVE statuses while C1's resume tiers still filtered on `in-development` alone — the write side landed a commit before the read side, so three of the four statuses were writable but invisible. Both tiers are widened in commit 3's contract edits; **without AT-7 that widening can land and silently not work**, because A-CAP-2 exercises `in-development` only and would pass either way. A test that cannot distinguish the fixed state from the broken one is not coverage. Add a negative control: a fifth fixture at a TERMINAL status (`rejected`) must **not** appear.
...
**The five A-HAND tests are one matrix, not five unrelated cases.** Step 1.0 routes on three field-presence branches (neither · exactly one · both) and then applies four checks, so the matrix is: no fields (2) · exactly one field (5) · both fields with a good record (1) · both fields with a record that cannot be resolved (3) · both fields with a record that resolves but disagrees or is terminal (4). Amended 2026-07-28 (S6-ceb) when Step 1.0 was tightened to verify provenance rather than trust two field labels; A-HAND-3, A-HAND-4 and A-HAND-5 are new, and A-HAND-1 gained its verification-visible clause. **A-HAND-5 exists because the first draft of this matrix claimed to cover "the whole input space" while omitting the exactly-one-field branch entirely** — caught by `/qc-pass`, and recorded here because a completeness claim is the kind of thing that is believed rather than checked.

- **A-HAND-1 — Valid handoff: no double qualification.** Brief carrying `**Capability:**` + `**Settled upstream:**`, pointing at a real record at `projects/{p}/development/{slug}.md` whose frontmatter `capability:` matches the brief's slug, whose `owner_project:` matches the `{p}` in its own path, and whose `status:` is in the ACTIVE set. **Pass:** all four Step-1.0 checks pass; the command states in one line which record path it trusted and that record's `status:`; treats 1.1–1.2 as satisfied; runs 1.3–1.6 on the artifact.

  **The disposition-return clause of this test is `unassessed until commit 5`, not passed.** "Returns its disposition to the calling unit rather than making an adoption decision" requires a calling unit, and no producer exists until commit 5 (`.claude/commands/work-loop.md` Step 5b stubs the capability route). Commit 6 recorded A-HAND-1 as passed; the resolvable half — the checks, the trust line, 1.1–1.6 behaviour — genuinely was, and this half was necessarily asserted rather than observed. Marked here per A-CAP-6's own rule that an untested claim is `unassessed`, never `passed`.
- **A-HAND-2 — Ordinary direct invocation unchanged.** Plain need, neither capability field. **Pass:** full Step 1 including 1.1–1.2, identical to pre-commit-6; Step 1.0 emits **nothing at all** — no validation line, no malformed-handoff report. Silence is part of the pass condition: the clause must not add noise to the command's ordinary path.
- **A-HAND-3 — Missing record: the bypass does not fire.** Both fields present, but `**Capability:**` names a record that does not exist. Run three sub-cases: (a) a bare slug matching **zero** files under `projects/*/development/`; (b) an explicit path to a non-existent file; (c) a bare slug matching **two or more** files, which is equally unresolvable. **Pass, all three:** reported as a **malformed upstream handoff** naming which check failed and what was observed; **1.1 and 1.2 run in full**; **no capability record is created and none is repaired**; the need is not inferred from the brief alone.
- **A-HAND-4 — Resolvable but not trustworthy: identity or status fails.** Both fields present and a real file resolves, but a later check fails. Run four sub-cases: (a) the record's frontmatter `capability:` differs from the slug the brief supplied *(check 3)*; (b) the record's `owner_project:` differs from the `{p}` segment of its own path *(check 3)*; (c) the resolved file is **not** under a `development/` directory, or carries no YAML frontmatter with a `capability:` key — i.e. some other document with a plausible name *(check 2; reachable only via the explicit-path form, since the bare-slug glob is already rooted at `projects/*/development/`)*; (d) every identity check agrees but `status:` is TERMINAL — `rejected`, `adopted`, `closed`, `retired` or `keep-local` *(check 4)*. **Pass, all four:** reported as a **malformed upstream handoff** naming which check failed and what was observed versus expected; **1.1 and 1.2 run in full**; the record is **not** edited to make it agree.

  Sub-case (c) is the forgery case stated plainly: a document that merely *carries the two labels* and points at a plausible path must not buy a qualification bypass. Sub-case (d) is the subtler one — a `rejected` record is kept forever precisely as evidence the need was **refused**, so honouring it would let a document that says "no" be read as upstream validation that says "yes."

  Verified 2026-07-28 that no *executable* component emits these fields (`command grep -rn "Settled upstream"` across `ai-resources/` returns only documentation, plans and audit reports — this plan included — and no command, skill or script), so **until commit 5 specifies the producer side in `docs/work-loop.md`, every brief reaching this clause is by definition not from `/work-loop`** — which is what makes A-HAND-3, A-HAND-4 and A-HAND-5 the load-bearing group, not edge cases.

- **A-HAND-5 — Half-formed handoff: exactly one field.** Two sub-cases: (a) `**Capability:**` present, `**Settled upstream:**` absent; (b) the reverse. **Pass, both:** reported as a **malformed upstream handoff**; **1.1–1.6 run in full**; no check-1 resolution is attempted on the basis of one field alone, and no record is created. This branch is what stops a brief that is *partly* shaped like a handoff from being treated as either a clean handoff or a clean direct invocation.

#### Producer-side obligations this creates for commit 5

Step 1.0 ships as a **fail-closed consumer before its producer exists** — deliberately, so the bypass cannot be exercised while unguarded. That ordering is sound, but it has already constrained commit 5 in three ways, recorded here rather than in the command so the command stays runtime-only and does not go stale the moment commit 5 lands.

1. **`**Capability:**` must carry a bare slug or a workspace-relative path, and nothing else.** The **composite** form used consistently across all four superseded drafts — `**Capability:** {slug} — owner {project} — record: {path}` (`develop-capability-build-plan.md:846`, `-v2.md:872`, `-v3.md:864`, `-v3.1.md:904`) — **is rejected by check 1**, and would surface as a *provenance failure* rather than a format mismatch, which is the hardest kind to diagnose. If commit 5 prefers the composite, it must amend check 1 **in the same commit**.
2. **The brief format must be specified in `docs/work-loop.md`.** Today neither C1 nor C3 states that a routed-out brief carries these labels at all; the contract exists only in the consumer.
3. **The routed-out return address must be reconciled.** `docs/work-loop.md:157` defines `routed-out` as ownership having *"left the loop terminally"* and `:171` marks the unit `Status: complete`, while `.claude/commands/work-loop.md:124` says *"Return its disposition to this unit"*. **These two shipped files contradict each other** — the `/scope-project` bullet at `docs/work-loop.md:49` carries an explicit "Terminal exit" marker and the `/develop-ai-resource` bullet at `:48` does not, so the generic terminal definition at `:157` over-reaches. Step 1.0 and Step 4 sidestep it by keying the disposition on the **capability record** (durable, open while the capability is in development) rather than on the unit. Commit 5 must decide whether a `/develop-ai-resource` handoff is terminal or resumable and make C1 and C3 agree. **Until it does, A-HAND-1's disposition-return clause stays `unassessed`** — it is not merely unobserved, it is currently unsatisfiable as C1 words it.
