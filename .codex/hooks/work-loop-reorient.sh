...
jq -cn --arg cwd "$CWD" --arg common "$COMMON_DIR" '{
  hookSpecificOutput: {
    ...
    additionalContext: (
      ...
      " (git common dir: " + $common + "). Before your next move, re-read from disk the active Work Loop pointers preserved under AGENTS.md § Compaction: the exact active logs/work-loop/{task-id}.md path, the bound checkout, the governing plan path with its workflow and phase, and the current ## Next action. Do not continue from the compacted summary. If those pointers did not survive the compaction, say so — do not go looking for the task."
    )
  }
}' 2>/dev/null || exit 0
